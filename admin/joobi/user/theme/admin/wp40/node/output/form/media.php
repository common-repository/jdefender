	...
	function create() {
		...
		if ( !$onlyURL ) {
			...
			if ( $fancyFileUpload ) {
				...
				$fielUploadURL = WPages::linkAjax( 'controller=output&task=uploadfile&wajx=1' );
					$mytranslation = array();
...
dropArea:'#drophere_" . $countUploadsN . "',";
		...
		if ( $this->_maxFiles < 2 ) {
				...
				if ( $mimeA[0]=='image' && !$myFileInfoO->secure ) {
					...
					$url = $myNewImageO->fileURL( $myNewImageO->thumbnail );
					$this->content .= '<div style="float:left; clear:both;"><div><strong>' . WText::t('1298350386KTQN') . '</strong></div>';
					...
					$this->content .= '<div style="padding: 5px 0 10px;">' . $myFileInfoO->filename . '</div></div>';
				} else {
