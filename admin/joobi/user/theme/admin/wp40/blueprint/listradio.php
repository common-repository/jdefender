	...
	public function render($data) {	
		...
		foreach( $data->listA as $key => $value ) {
			...
			if ( substr( $listValue, 0, 2 ) == '--' ) {
				...
				if ( $data->disable ) $extra .= ' disabled="disabled"';	
				...
				if ( !empty($data->disable) ) $html .= ' disabled="disabled"';
		...
		switch( $data->radioStyle ) {
			...
			case 'checkBoxMultipleSelect':
				...
				$style = 'height:' . $heightBigBox . 'px;';
				if ( $data->disable ) {
					$style .= 'color:gray;';
				}
				$html = '<div class="checkBoxMultipleSelect" style="' . $style . '">' . $html . '</div>';
				break;
			case 'radioButton':
			default:
