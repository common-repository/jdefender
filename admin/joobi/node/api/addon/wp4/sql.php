...
if( is_numeric($rs))$RA[]=$rs;
else $RA[]=stripslashes($rs );
}
return $RA;
}
public function loadObject(){
$result=$this->_dbInstance->get_row($this->_query );
if(empty($result)) return $result;
$oO=new stdClass;
foreach($result as $k=> $r){
if( is_numeric($r))$oO->$k=$r;
else $oO->$k=stripslashes($r );
}
return $oO;
}
public function loadObjectList(){
$indexResult=is_string($this->_indexResult )?$this->_indexResult : '';
$result=$this->_dbInstance->get_results($this->_query );
if(empty($result)) return $result;
$RA=array();
foreach($result as $rs){
$oO=new stdClass;
...
$query .=' ( `id` INT )';
$query .=' ENGINE='.$engine.' /*!40100 DEFAULT CHARACTER SET '.JOOBI_DB_CHARSET.' COLLATE '.JOOBI_DB_COLLATE.'*/;';
return $query;
}
