...
$roleC=WRole::get();
if(!$roleC->compareRole($extraPrams->rolid, 'registered')){
return 0;
}}
$data=array();
...
$sendURL=WPages::linkAjax($sendURL );
}
...
if(!IS_ADMIN && 'contacts'==WPref::load('PUSERS_NODE_FRAMEWORK_FE')){
$scripName=WGlobals::get('SCRIPT_NAME','','server');if(empty($scripName)){
$scripName=WGlobals::get('PHP_SELF','','server');
}
$exA=explode('/',$scripName );
$scrpt=array_pop($exA );
if('wp-login.php'==$scrpt && 'register'==WGlobals::get('action')){
$usersAddon=WAddon::get('users.contacts');
$link=$usersAddon->goRegister();
}
}
return true;
}
...
$myTime=WTools::timeOut();
$expiredTime=time() - $myTime; $sessionM->noValidate();
