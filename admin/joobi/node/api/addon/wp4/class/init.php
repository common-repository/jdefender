...
if(!IS_ADMIN){
WApplication_wp4::createFunction( JOOBI_PREFIX.'_jscss_scripts','init','addFrontEndScripts');
add_action('wp_enqueue_scripts', JOOBI_PREFIX.'_jscss_scripts');
...
add_action('wp_ajax_nopriv_'.'jbiwpx', JOOBI_PREFIX.'_handle_ajax');
$this->_userHooks();
...
wp_enqueue_script('jquery',$path, array(), JOOBI_VERSION );
}
$path=WPage::fileLocation('js','js/rootscript.1.2.js','api');
$path='/'.ltrim($path , JOOBI_SITE_PATH );
wp_enqueue_script('rootscript',$path, array(), JOOBI_VERSION );
$path=WPage::fileLocation('js','js/themescript.1.2.js');
$path='/'.ltrim($path , JOOBI_SITE_PATH );
wp_enqueue_script('themescript',$path, array(), JOOBI_VERSION );
if(!empty(APIPage::$jsFileA)){
foreach( APIPage::$jsFileA as $name=> $header){
wp_enqueue_script( md5($header), $header, array(), JOOBI_VERSION );
}
