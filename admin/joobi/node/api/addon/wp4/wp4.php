...
define('JOOBI_DB_CHARSET','utf8');
define('JOOBI_DB_COLLATE','utf8_general_ci');
...
$js=str_replace( JOOBI_SITE, '/',$header );
if('http'==substr($js, 0, 4 )){
APIPage::$jsFileA[]=$js;
return;
}
}elseif('//'==substr($header, 0, 2 )){
APIPage::$jsFileA[]=$header;
return '';
}else{
$js=str_replace( JOOBI_SITE_PATH, '/',$header );
}
APIPage::$jsFileA[]=rtrim( JOOBI_SITE, '/'). $js;
}
public static function addStyleSheet($header,$type='text/css',$media=null,$attributes=array()){
static $isPopUp=null;
if('http'==substr($header, 0, 4 )){
$header=str_replace( JOOBI_SITE, '/',$header );
}else{
$header=str_replace( JOOBI_SITE_PATH, '/',$header );
...
$rand='w'.rand( 100000, 999999 ). '-'.time();
...
$mail->mailer='mail';$mail->sendmail='/usr/sbin/sendmail';$mail->smtpauth=get_option('admin_email');$mail->smtpsecure=false;$mail->smtpport=get_option('mailserver_port');$mail->smtpuser=get_option('mailserver_login');$mail->smtppass=get_option('mailserver_pass');$mail->smtphost=get_option('mailserver_url');
return $mail;
}
...
define('JOOBI_DS_THEME_JOOBI', JOOBI_DS_USER.'theme/admin/'.WPage::cmsDefaultTheme().'/');
}
...
if( WGlobals::getSession('page','embed', false)){
$itemId=false;
$indexPassed='popup';
}
$link=trim($link);
...
$fullOption=($foption !=null )?$foption : $currentOption;
if(!empty($page_id)){
$link=$absoluteLinkNewLink . JOOBI_INDEX.'?'. $page_id.'&'.$link.'&isPopUp=true'.URL_NO_FRAMEWORK;
}else{
$link=$absoluteLinkNewLink . JOOBI_INDEX.'?page='. $fullOption.'&'.$link.'&isPopUp=true'.URL_NO_FRAMEWORK;
}
}elseif($index=='link'){
...
var iframe=jQuery(modal).find('iframe');
jQuery(iframe).css('visibility','hidden');
jQuery(iframe).change(function(){
...
if($onlyOnce){
if( IS_ADMIN || WGlobals::get('isPopUp')){
...
if(!is_string($url)) return JOOBI_MAIN_APP;
$app=strtolower($url );
return $app;
}
public static function name($short='default',$pageID=null,$linkController=null){
$myOption=WApplication::getApp();
$joptionVal=array('jvendor','juser','jcommunity','jiptracker','jnotes','jprojects','jlinks','jdesign','jfaq','jdiscuss','jservicesprovider','jdefender','japps',
'jstore','jfeedback','jcomment','jtickets','jcloner','jaffiliates','japplication','jbackup','jstudio','jmobile','jmarket','jauction','jvouchers', 'jsubscription','jtestcases','jcontacts','jcatalog','jdownloads','jdloads','jclassifieds','jdatabase','jtodo','jchecklist','jdocumentation','jdistribution','jbounceback','jdictionary','jforum','jlicense','jnewsletters','jnews','jcampaign','joomfusion');
...
$availLangs=array();
foreach($languages as $language){$lg=str_replace('_','-',$language );
$exA=explode('-',$lg );
$availLangs[]=$exA[0];
}
...
WLoadFile('helper', JOOBI_DS_NODE.'api/addon/'.JOOBI_FRAMEWORK.'/');
...
$fileS=WGet::folder();
$exist=$fileS->exist( WP_PLUGIN_DIR.'/'.$component );
if($exist){
...
public static function renderJS($reset=true){
$noheader=WGlobals::get('noheader');
$jsHTML='';
if($noheader){
...
if($onlyOnce){
WPref::get('library.node', true, true, true);
$onlyOnce=false;
}
if( IS_ADMIN){
...
if($isPopUp){
WGlobals::set('is_popup', true, 'global');
WGlobals::set('noheader','1');
}else{
WGlobals::set('is_popup', false, 'global');
}
$device=WGlobals::get('device', false);
if( WGlobals::get('embed') || $device=='mobile'){
WGlobals::setSession('page','embed', true);
}
if( IS_ADMIN ) WPage::addJSScript( "jCore.home='" . JOOBI_SITE . "wp-admin/';" . WGet::$rLine );
else WPage::addJSScript( "jCore.home='" . JOOBI_SITE . "';" . WGet::$rLine );
}
...
WGet::loadLibrary();
$ref=WGlobals::get('HTTP_REFERER','','server','string');
if( strpos($ref,  JOOBI_FOLDER.'/inc/builder')) exit;
self::_wpInit();
...
if(!isset($option))$option=get_option('permalink_structure');
if( JOOBI_USE_SEF){
$requestURI=WGlobals::get('REQUEST_URI','','server','string');
$requestURI=trim($requestURI, '/');
if( strpos($requestURI, '/') !==false){
$expURIA=explode('/',$requestURI );
$lastPart=array_pop($expURIA );
$menuM=WTable::get('posts','','ID');
$menuM->where('post_parent','!=', 0 );
$menuM->whereE('post_name',$lastPart );
$contentA=$menuM->load('ol',array('post_content','post_parent','ID'));
if(!empty($contentA)){
if( count($contentA ) <=1){
$tag=trim($contentA[0]->post_content );
}else{
$secondPart=array_pop($expURIA );
$menuM=WTable::get('posts','','ID');
$menuM->where('post_parent','!=', 0 );
$menuM->whereE('post_name',$secondPart );
$secondParent=$menuM->load('lr','ID');
$menuM=WTable::get('posts','','ID');
$menuM->whereE('post_parent',$secondParent );
$menuM->whereE('post_name',$lastPart );
$tag=$menuM->load('lr','post_content');
$tag=trim($tag );
}}}
}
...
$currentURL=WGlobals::get('REQUEST_URI', null, 'server');
if(empty($currentURL))$currentURL=WGlobals::get('SCRIPT_URL', null, 'server');
if(empty($currentURL))$currentURL=WGlobals::get('REDIRECT_URL', null, 'server');
if(!empty($currentURL)){
