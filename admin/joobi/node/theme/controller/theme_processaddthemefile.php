...
if($fileClass->exist(JOOBI_DS_THEME.$destfolder. '/'.$folder. '/'.$themeFiletype. '/'.$filename.'.'.$fileEextension)){
$message=WMessage::get();
$message->userE('1307005978SBLO');
$this->setView('theme_add_file');
return true;
}
$fileClass->write(JOOBI_DS_THEME.$destfolder. '/'.$folder. '/'.$themeFiletype. '/'.$filename.'.'.$fileEextension, $content, 'overwrite');
return true;
}}