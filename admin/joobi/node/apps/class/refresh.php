...
if($appExist){
$appPref=WPref::get('apps.node');
$appPref->updatePref('hasupdate', 1 );
if(!empty($myAdmins)){
$mail=WMail::get();
$mail->keepAlive();
...
$mainDistrib=WPref::load('PINSTALL_NODE_MAINDISTRIB');
$netcom=WNetcom::get();
$receivedData=$netcom->send($mainDistrib, 'repository','checkDistribSite',$sentData );
$hasUpdate=false;
...
if(!in_array($level,$extension->levels))$new_levels[]=$level;
}
}else{
...
$lwid_convert=array();
$appAvaiableA=array();
foreach($this->data[0] as $userinfo){
$appAvaiableA[]=$userinfo['namekey'];
}
$appsM=WModel::get('apps');
$appsM->whereIn('namekey',$appAvaiableA );
$widAvaiableA=$appsM->load('lra','wid');
$appsUserinfosM=WModel::get('apps.userinfos');
$appsUserinfosM->whereIn('wid',$widAvaiableA );
$appsUserinfosM->setVal('enabled', 1 );
$appsUserinfosM->update();
$appsUserinfosM->whereNotIn('wid',$widAvaiableA );
$appsUserinfosM->setVal('enabled', 2 );
$appsUserinfosM->update();
...
$appsUserinfosM->whereIn('wid',$wid_convert );
$hasUserInfoA=$appsUserinfosM->load('lra','wid');
$insertWIDA=array();
foreach($wid_convert as $wid){
if(!in_array($wid, $hasUserInfoA )){
$insertWIDA[]=array($wid, 1 );
}}
$appsUserinfosM->setIgnore();
if(!empty($insertWIDA))$appsUserinfosM->insertMany( array('wid','enabled'), $insertWIDA );
...
WMessage::log('_updateForeignKeysValues()','UPDATE_FOREIGN_KEY_CHECK');
...
$sql->makeLJ('library.table','dbtid', 0, 1 ); $sql->makeLJ('library.columns','map','name', 0, 2 ); $sql->makeLJ('library.table','ref_dbtid','dbtid', 0, 3 ); $sql->makeLJ('library.columns','map2','name', 0, 4 ); $sql->whereE('dbtid',$model->getTableId());
