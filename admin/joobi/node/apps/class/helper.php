...
switch($type){
case 'component':
$adminFolder=JOOBI_DS_ROOT.'administrator/components/'.$folderName;
$siteFolder=JOOBI_DS_ROOT.'components/'.$folderName;
$folder=$adminFolder;
...
break;
case 'plugin':
$siteFolder=JOOBI_DS_ROOT.'plugins/'.$group.'/';
if(!$systemFolderC->exist($siteFolder)) return false;
...
break;
case 'module':
$folder=JOOBI_DS_ROOT .($group?'' : 'administrator/'). 'modules/'.$folderName.'/';
if(!$systemFolderC->exist($folder)) return false;
$files=$systemFolderC->files($folder,'!!\.xml$',true,true);
break;
case 'template':
$folder=JOOBI_DS_ROOT .($group?'' : 'administrator/'). 'templates/'.$folderName.'/';
if(!$systemFolderC->exist($folder)) return false;
$files=$systemFolderC->files($folder,'!!\.xml$',true,true);
break;
default:
...
require(JOOBI_DS_ROOT.'libraries/joomla/application/helper.php');
}
$version=false;
