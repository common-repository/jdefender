...
do {
$count++;
$this->username=strtolower( WTools::randomString( 10, false));
...
$onRegsitration=WGlobals::get('userOnRegister', false, 'global');
...
$onRegsitration=WGlobals::get('userOnRegister', false, 'global');
if($onRegsitration){
$registerRole=WPref::load('PUSERS_NODE_REGISTRATIONROLE');
if(!empty($this->rolid)){
if($this->_roleC->compareRole($this->rolid, $registerRole )){
$this->rolid=$registerRole;
}}else{
$this->rolid=$registerRole;
}}
$isRegister=$this->_roleC->compareRole($this->rolid, 'registered');
...
if( WPref::load('PCONTACTS_NODE_ORGANIZATION') && !empty($this->_x['organization'])){
$organizationApiO=WObject::get('organization.api');
$organizationApiO->create($this->_x['organization'] );
$organizationApiO->linkUser($this->uid );
}
}
}
WHook::addAction('onAppRender','users.register','notifyAdmin',$this->uid );
$this->_updateCMSUser();
return true;
}
function editExtra(){
...
case 'flexible':
if(!empty($this->uid )){
$usersRegisterC->sendEmailValidation($this->uid, $password );
}break;
default:
