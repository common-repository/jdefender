...
class Files_savefile_controller extends WController {
	function savefile() {
		...
		if ( $fancyFileUpload ) {
			...
			$map = 'xfile';
			if ( !isset($files['type'][0][0][$map]) ) {
				$this->adminE( 'Column not found!' );
				return false;
			}			
		...
		$filesM->returnId();
		$status = $filesM->save();
		if ( ! $status || empty($filesM->filid) ) {
			$this->adminE( 'The file could not be saved!' );
			return true;
		}
		$filid =  $filesM->filid;
		...
		if ( $status ) $this->userS('1318337621NXST');
		else $this->userE('1314849832QOQC');
		$cacheC = WCache::get();
