...
$this->audioHeight = 70;			
	...
	function uploadDirectory($uploadFiles) {
		...
		$uploadFiles->directory = JOOBI_DS_ROOT . $uploadFiles->directory . '/';
				$uploadFiles->directory =  preg_replace('#[/\\\\]+#', '/', $uploadFiles->directory);
		...
		$directoryto = JOOBI_DS_USER . $folders . '/' . $path;
		...
		if ( $filenames ) {
			for( $n=0; $n<count($filenames); $n++ ) {
				...
				if ( in_array($ext , $uploadFiles->extension) || empty($uploadFiles->extension)) {
					...
					$destination = $directoryto . '/' . $filename;
					$filesM = WModel::get('files');
