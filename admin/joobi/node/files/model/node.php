...
class Files_Node_model extends WModel {
	...
	public function saveOneFile($fileContent,$fileName,$filePath='',$basePath='',$overwrite=false) {
		if ( !empty($filePath) ) {
			$this->_path = str_replace( array( '|', '/', '\\'), '/', $filePath );
		}
		if ( !empty($basePath) ) {
			$this->_folder = trim( str_replace( JOOBI_DS_USER, '', $basePath ), '/' );
		}
		$this->_name = $fileName;
		$this->thumbnail = 0;
		$this->secure = 0;
		...
		$this->size = $file->write( $this->_completePath . '/' . $this->name . '.' . $this->type, $fileContent, $writeType );
		return $this->filid;
	}
	...
	function deleteExtra($eid=0) {
		if ( !empty($this->_x->path) || !empty($this->_x->secure) ) {
			...
			else $this->_basePath = rtrim( $this->_basePath, '/' ) . '/';
			$this->_x->path = str_replace( '|', '/', $this->_x->path );
			$fileC = WGet::file( $this->_x->storage );
			$fileC->setFileInformation( $this->_x );
			$this->_completePath = $fileC->makePath( $this->_x->folder, $this->_x->path ) . '/';
			if ( !empty($this->_x->thumbnail) ) {
				$thumbnail = $this->_completePath . 'thumbnails/' . $this->_x->name . '.' . $this->_x->type;
	...
	public function move() {
		...
		if ( $this->_noFileJustContent ) {
			return $file->write( $this->_tmp_name, $this->_completePath . '/' . $this->name . '.' . $this->type );
		} elseif ( $this->_copy ) {
			return $file->copy( $this->_tmp_name, $this->_completePath . '/' . $this->name . '.' . $this->type );
		} elseif ( $this->_uploadFile ) {
			return $file->upload( $this->_tmp_name, $this->_completePath . '/' . $this->name . '.' . $this->type );
		}  else {
			return $file->move( $this->_tmp_name, $this->_completePath . '/' . $this->name . '.' . $this->type );
		}
	}
	public function write($content,$destination=null) {
		$file = WGet::file( $this->_storage );
		$filename = $this->name . '.' . $this->type;
		$LOCATION = rtrim( $destination, '/' ) . '/' . rtrim($this->path, '/' ) . '/' . $filename;
	...
	public function getFilePath() {
		if ( $this->secure ) {
			$path = JOOBI_DS_USER . 'safe/' . $this->convertPath( $this->path ) . '/' . $this->name . '.' . $this->type;
		} else {
			$path = JOOBI_DS_USER . $this->convertPath( $this->path ) . '/' . $this->name . '.' . $this->type;
		}
		return $path;
	}
	private function _checkUnicity($unique=true,$external=false) {
		...
		while ( $result ) {
			if ( $unique ) {	
				if ( !$file->checkExist() || !$file->exist( $this->_completePath . '/' . $fileName .'.'. $this->type ) ) return true;
			} else {	
				$this->whereE('name', $this->name);
				$this->whereE('path', $this->path);
				$this->whereE('type', $this->type);
