...
define('JOOBI_DS_JOOBI' , JOOBI_DS_ROOT . JOOBI_FOLDER.'/'  );
define('JOOBI_VERSION' , 1706 ); 
...
define('JOOBI_DS_USER', JOOBI_DS_JOOBI.'user/');
define('JOOBI_DS_USERS', JOOBI_DS_JOOBI.'users/');
define('JOOBI_DS_MEDIA', JOOBI_DS_USER.'media/');
define('JOOBI_DS_THEME', JOOBI_DS_USER.'theme/'); define('JOOBI_DS_EXPORT', JOOBI_DS_USER.'export/');
define('JOOBI_DS_TEMP', JOOBI_DS_ROOT.'tmp/');
define('JOOBI_LIB_HTML' , JOOBI_DS_NODE.'output/');
define('JOOBI_LIB_HTML_CLASS' , JOOBI_DS_NODE.'output/class/');
if(!defined('JOOBI_DS_INC')) define('JOOBI_DS_INC', JOOBI_DS_JOOBI.'inc/');
require( JOOBI_LIB_CORE.'class/abstract.php');
require( JOOBI_LIB_CORE.'class/requete.php');require( JOOBI_LIB_CORE.'class/table.php');
require( JOOBI_LIB_CORE.'class/model.php');
require( JOOBI_LIB_CORE.'class/controller.php');
require( JOOBI_LIB_CORE.'class/globals.php');
require( JOOBI_LIB_CORE.'class/get.php');
require( JOOBI_LIB_CORE.'class/tools.php');require( JOOBI_LIB_CORE.'class/preferences.php');
require( JOOBI_LIB_HTML_CLASS .'node.php');require( JOOBI_LIB_HTML_CLASS .'html-tools.php');
require( JOOBI_LIB_CORE.'class/messages.php');
...
if( in_array( JOOBI_FRAMEWORK_TYPE, array('joomla','wordpress')) && defined('JOOBI_DS_THEME_JOOBI') && $base==JOOBI_DS_THEME_JOOBI){
$hasOverwrite=WView::themeIsOverWritten();
if($hasOverwrite){
$overwriteExist=WLoadFile($filePath, WPage::getTemplate('path'). '/joobi',$expand, false);
...
$rt=str_replace( "/", "\\", JOOBI_DS_ROOT );
$base=str_replace( "/", "\\", $base );
$base=str_replace($rt, '',$base );
}else{
$base=str_replace( JOOBI_DS_ROOT, '',$base );
}
$key4Static=$base . str_replace('.','/',$filePath );
...
$path=JOOBI_DS_ROOT . $base . implode( DS, $parts ). '/class/'.$filename.'.php';
...
case 'action':
$filename=array_pop($parts );
$path=JOOBI_DS_ROOT . $base . implode( DS, $parts ). '/'.$type.'/'.$filename.'.php';
break;
case 'controller': $path=JOOBI_DS_ROOT . $base . implode( DS, $parts ). '/controller.php';
break;
default:
$path=JOOBI_DS_ROOT . $base . str_replace('.','/',$filePath ). '.php';
break;
}
if( file_exists($path)){
if(!empty($className)){
...
function debugPath($useDebug=true){
if($useDebug ) return debug( 91827364, debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS ));
else debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS );
}
