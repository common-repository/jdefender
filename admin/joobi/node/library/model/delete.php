...
$limith=$localObj->_limith;
if(empty($tempWhere)){
WMessage::log('WModel_Delete','delete_without_condition');
WMessage::log($localObj , 'delete_without_condition');
return true;
}
}
...
if(empty($eid)){$localObj->resetAll();
$localObj->_whereValues=$tempWhere;
$localObj->_leftjoin=$leftJoin;
if(!empty($limith))$localObj->setLimit($limith );
else $localObj->setLimit( 500000 );
...
$databaseForeignM->makeLJ('library.columns','map','name', 0, 1 );
$databaseForeignM->makeLJ('library.model','dbtid','dbtid', 0,2 );
...
$message=WMessage::get();
$message->codeE('1. Error deleting the items for the model: '.$sql->getModelNamekey());
continue;
}}}else{
$sql->select($fk->map );
$sql->setLimit( 1000 );
...
if(!$sql->deleteAll()){
$message=WMessage::get();
$message->codeE('2. Error deleting the items for the model: '.$sql->getModelNamekey());
continue;
}}
}
}}
}
$status=$localObj->delete($eid, true);
if($status){
if( method_exists($localObj, 'deleteExtra')){
$localObj->deleteExtra($eid );
}}
return $status;
}
return false;
