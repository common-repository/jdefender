...
if( substr($modelNamkey, -5 ) !='trans' && substr($modelNamkey, -5 ) !='.type'){
...
$table=str_replace('.','_', substr($localObj->_infos->dbtid, 1 ));
$tableLd=str_replace('#','.',$table );
$tempdata=WGet::loadData($tableLd, 'table');
if(!empty($tempdata->columnsA )){
$allAutoFields=array();
foreach($tempdata->columnsA as $column){
$colO=new stdClass;
$colO->name=$column['name'];
$colO->columntype=$column['columntype'];
$allAutoFields[]=$colO;
}}
}
...
if($localObj->_new){
if(isset($autoFields->author) && empty($localObj->author )){
if(!empty($localObj->_update))$localObj->setVal('author',$user->uid );
else $localObj->author=$user->uid;
}if(isset($autoFields->uid) && $localObj->getPK() !='uid' && empty($localObj->uid )){
if(!empty($localObj->_update))$localObj->setVal('uid',$user->uid );
$localObj->uid=$user->uid;
}if(isset($autoFields->namekey) && empty($localObj->namekey)){
if(!empty($localObj->_update))$localObj->setVal('namekey',$localObj->genNamekey());
$localObj->namekey=$localObj->genNamekey();
}if(isset($autoFields->created) && empty($localObj->created)){
if(!empty($localObj->_update))$localObj->setVal('created', time());
else $localObj->created=time();
}} else unset($localObj->created);
if(isset($autoFields->modifiedby) && ( !isset($localObj->modifiedby ) || $localObj->modifiedby<1 )){
if(!empty($localObj->_update))$localObj->setVal('modifiedby',$user->uid );
else $localObj->modifiedby=$user->uid;
}if(isset($autoFields->modified) && ( !isset($localObj->modified ) || $localObj->modified<1 )){
if(!empty($localObj->_update))$localObj->setVal('modified', time());
else $localObj->modified=time();
}
if(!empty($localObj->_infos->fields)){
$this->_processCustomFields($localObj );
}
}
if(!empty($localObj->namekey) && $localObj->_checkNamekey){
...
$model->setLimit( 500 );
$existingValuesA=$model->load('lra');
...
$myValueToInsert=array_diff($myValueToInsert, $existingValuesA );
$myValueToDelete=array_intersect($myValueToDelete, $existingValuesA );
}else{
...
$model->setLimit( 500 );
$existingValuesA=$model->load('lra');
if(!empty($existingValuesA)){
$myValueToDelete=array_diff($existingValuesA, $allValuesToProcess );
$myValueToInsert=array_diff($allValuesToProcess, $existingValuesA );
}else{
$myValueToInsert=$allValuesToProcess;
}
}
...
$sql->makeLJ('library.columns','map','name', 0, 1 );
$sql->makeLJ('library.model','dbtid','dbtid', 0,2 );
...
$tempFileM->_basePath=JOOBI_DS_USER . $localObj->_fileInfo[$arrKey]->folder.'/';
