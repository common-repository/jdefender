...
$status=true;
$trkA=$this->_loadFromTruc($localObj );
if(!empty($localObj->config)){
foreach($localObj->config as $cfgK=> $cfgV){
$current=WGet::readConfig($cfgK );
if($current==$cfgV ) continue;
WGet::saveConfig($cfgK, $cfgV );
}
}
...
$existVal=WPref::load($mypref );
if(!is_numeric($existVal)){
$existVal=(string)$existVal;
}
if($existVal !=$val){
if(!isset($preference)){
