...
if( class_exists($extClassName )) eval($extendedClass );
}
}}
$newClass=null;
if($tempdata->path){
...
if(!empty($tempdata->custom))$base=JOOBI_DS_USER.'custom/'.$myFuntion.'/custom/';
...
$exptA=array('add','edit','gologin','login','registration');
if(!empty($truc) && ! in_array($task, $exptA )){
if(!self::verifySpoof()){
$message=WMessage::get();
...
if( WRoles::isAdmin('manager')){
if( WPref::load('PAPPS_NODE_HASUPDATE') && ( WPref::load('PAPPS_NODE_SHOWUPDATE') || WPref::load('PLIBRARY_NODE_DBGERR'))){
$LINKAPP='<a href="'.WPage::linkAdmin('controller=apps'). '">'.WText::t('1267607212GWJP'). '</a>';
$this->adminW('There are new update availables. Go to the list of '.$LINKAPP.' to install the udpates!');
}}
}
...
$yidReturn=$this->yid;
$crtyid=WGlobals::get('crtyid','', null, 'int');
$yid2use=(!empty($crtyid)?$crtyid : $this->yid );
$id=WView::generateID('view', WGlobals::get('controller','', null, 'task'). '_'.WView::get($yid2use, 'namekey'));
$controller=null;
}
if(empty($vWdjx)){
...
$message=WMessage::get();
$message->cleanBuffer('Controller_Ajax_return', true);
echo $status;
exit;
}
}else{
...
if('mobile' !=JOOBI_FRAMEWORK_TYPE && 'netcom' !=JOOBI_FRAMEWORK_TYPE && empty($wmjx )){
...
foreach( self::$ajaxRturnA as $ajaO){
if('redirect'==$ajaO->act){
$hasRedirect=true;
break;
}}}
$message=WMessage::get();
$hanckerToUse='#WMessage';
$wmssid=WGlobals::get('wmssid');
if(!empty($wmssid))$hanckerToUse .=$wmssid;
if($hasRedirect){
$message->store();
self::ajaxReturnObj('replace',$hanckerToUse, '');
}else{
$userMess=$message->getM();
if(!empty($userMess)){
self::ajaxReturnObj('replace',$hanckerToUse, $userMess );
}else{
self::ajaxReturnObj('replace',$hanckerToUse, '');
}
}
$message=WMessage::get();
$message->cleanBuffer('Controller_Ajax_return', false);
if(!empty( self::$ajaxRturnA )){
...
$tempdata=WController::ooController($trigger, $action, true, false);
...
$tempdata=WController::ooController($controller, '', false, false);
...
if(!empty($this->viewNamekey)){
$this->yid=WView::get($this->viewNamekey, 'yid');
}
return true;
}
function autocheck(){
...
$task=WGlobals::get('task');
if( WPref::load('PLIBRARY_NODE_CHATBOX') && WRoles::isAdmin('manager') && 'mobile' !=JOOBI_FRAMEWORK_TYPE){
$mainChatC=WClass::get('main.chat', null, 'class', false);
if(!empty($mainChatC))$mainChatC->displayChatButton();
}
...
$viewEdit=WView::get($this->yid, 'useredit');
if(!IS_ADMIN && WRole::hasRole('admin') && JOOBI_APP_DEVICE_TYPE=='bw'){
if(  WPref::load('PMAIN_NODE_SHOWCONFIG') && ($viewEdit || WPref::load('PMAIN_NODE_PANELFORVIEWS'))){
...
if( WPref::load('PMAIN_NODE_SHOWVIEWSDETAILS') && ($viewEdit || WPref::load('PMAIN_NODE_PANELFORVIEWS'))){
$mainConfigC=WClass::get('main.confighelper', null, 'class', false);
if(!empty($mainConfigC )){
...
('show'==$task && 'apps'==$ctrl ) 
|| ('install'==$task && 'translation'==$ctrl )
|| ('design-export'==$ctrl )
|| ('backup-now'==$ctrl )
|| ('mailing-manual'==$ctrl )
)){
$js='jQuery(document).ajaxStart(function(){jQuery("body").addClass(\'wait\');});jQuery(document).ajaxComplete(function(){jQuery("body").removeClass(\'wait\');});';
WPage::addJSScript($js );
}
if(!IS_ADMIN && 'wordpress'==JOOBI_FRAMEWORK_TYPE){
$header='.entry-header{display:none;}';
echo '<style type="text/css" media="screen">'.$header . WGet::$rLine.'</style>'.WGet::$rLine;
}
if( in_array($task, array('listing','show','home','category'))){
...
if($showMesssage){
$ITEMNAME=$classOwnershipC->modelName();
if(!empty($ITEMNAME)){
$this->userE('1492016977MABV',array('$ITEMNAME'=>$ITEMNAME));
}else{
...
$wmjx=WGlobals::get('wmjx');
if($wmjx && empty($this->_toggleO->model)){
$myTrucs=WGlobals::get( JOOBI_VAR_DATA );
...
if( WGet::maintenanceMode())$caching=1;
$getModel=true;
if($caching > 5){
...
public function showM($status,$type,$NUMBER=1,$sid=0,$modelName=null){
$message=WMessage::get();
...
return self::_loadDataInfo($controller, $task, true, $showMessage );
}else{
$caching=WPref::load('PLIBRARY_NODE_CACHING');
...
if( WGet::maintenanceMode()){
$vocabExist=WGet::loadData('library.vocab','translation');
}
...
$a='<br /><p class="copyright">'.str_replace(' Core','',$extInfo->name );if('joomla'==JOOBI_FRAMEWORK_TYPE)$a .=' <a href="https://joobi.co" target="_blank">Joomla'.' '.'extensions</a>';
elseif('wordpress'==JOOBI_FRAMEWORK_TYPE)$a .=' <a href="https://joobi.co" target="_blank">Wordpress'.' '.'plugin</a>';
else $a .=' <a href="https://joobi.co" target="_blank">Joobi '.' '.'App</a>';
...
$tempdata=WController::getSQLFromDB($controller, $task );
if(empty($tempdata) || !empty($tempdata->reload)){
if(!is_numeric($cKey)){
$tempdata=WController::getSQLFromDB($controller, $task );
if(!empty($tempdata)){$controllerM=WModel::get('library.controller','object');
...
private static function _loadDataInfo($controller,$task,$recurse=true,$showMessage=true){
static $tempdataA=array();
$task=trim($task );
$namekey=str_replace('-','_',$controller ). (!empty($task)?'.'.$task : ''). ( WRoles::isAdmin('manager')?'.1' : '');
$ctrKey=$controller . (!empty($task)?'|'.$task : ''). ( WRoles::isAdmin('manager')?'|1' : '');
if(isset($tempdataA[$ctrKey])) return $tempdataA[$ctrKey];
$ctrA=explode('-',$controller );
$node=array_shift($ctrA );
if('setup'==$node){
WMessage::log($controller , 'data-file-not-found-C_now_A');
WMessage::log($task , 'data-file-not-found-C_now_A');
WMessage::log( debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS ) , 'data-file-not-found-C_now_A');
}
$namekeyCfg=str_replace( array('-','.'), '_',$namekey );
if(!empty($task)){
$tempdata=WGet::loadData($node.'.0_'.$namekeyCfg, 'controller');
if(empty($tempdata) && WRoles::isAdmin('manager')){
$namekeyCfg2=substr($namekeyCfg, 0, -2 );
$tempdata=WGet::loadData($node.'.0_'.$namekeyCfg2, 'controller');
}
if(empty($tempdata)){
$namekeyCfg=$controller . ( WRoles::isAdmin('manager')?'_1' : '');
$tempdata=WGet::loadData($node.'.1_'.str_replace('-','_',$namekeyCfg ), 'controller');
}
}else{
$tempdata=WGet::loadData($node.'.1_'.$namekeyCfg, 'controller');
if(empty($tempdata) && WRoles::isAdmin('manager'))$tempdata=WGet::loadData($node.'.1_'.substr($namekeyCfg, 0, -2 ), 'controller');
}
if(empty($tempdata) && $recurse){
$tempdata=self::_loadDataInfo($controller, '', false, $showMessage );
}
if(!empty($tempdata)){
...
if(!is_numeric($tempdata->yid))$tempdata->yid=WView::get($tempdata->yid, 'yid');
