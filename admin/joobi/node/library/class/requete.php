...
$dbgQry=( WPref::load('PLIBRARY_NODE_DBGQRY') || (defined('PLIBRARY_NODE_DBGQRY') && PLIBRARY_NODE_DBGQRY ));
...
if($dbgQry && !empty($uid)){
$show=true;
}elseif( WPref::load('PLIBRARY_NODE_DBGQRYGUEST') && ! IS_ADMIN && $uid==0){$show=true;
}
if($this->_debug
|| WGlobals::get('wz_debug_sql', false, 'global')
){
...
$content="\n\n".$content."\n\n";
if( WGet::readConfig('install')){$path=JOOBI_DS_ROOT.'error-queries.log';
}else{
$path=JOOBI_DS_USER.'logs'.'/'.$name.'.log';
}
...
echo $errorMessage2Show='The Query Failed with the error number: '.$number.' and message: '.$messageHTML.'The query is :<br />'. $queryHTML . WMessage::showLine('query', true);
}else{
