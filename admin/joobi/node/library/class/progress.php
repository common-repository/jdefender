...
var $useAjax=true;
var $detailsRefresh='';var $detailsAppend='';
...
$path=JOOBI_DS_USER . self::$_instanceA[$this->_name]->folder.'/'.self::$_instanceA[$this->_name]->_fileName;
return $path;
}
public function getFolderPath(){
$path=JOOBI_DS_USER . self::$_instanceA[$this->_name]->folder;
return $path;
}
...
$this->codeE('1. This case should not happen it means there is a problem with the steps define for the process : '.$this->_name.', step : '.print_r( self::$_instanceA[$this->_name]->_progressO, true));
}else{
...
if( method_exists($this, 'runStep'))$this->runStep();
else {
return false;
}
if(!empty($stepToRun) && $stepToRun !=self::$_instanceA[$this->_name]->_progressO->currentStep ) return true;
...
&& self::$_instanceA[$this->_name]->_progressO->status !='inprogress'
...
WMessage::log('currentStep : '.self::$_instanceA[$this->_name]->_progressO->currentStep  , 'install-progress-max-increment');
...
$achivement=self::$_instanceA[$this->_name]->_progressO->increment / $this->_stepTotalIncrementA[$step];
if($achivement > 1)$achivement=1;
$ratio +=($achivement * $stepRatio );
}else{
break;
}$count++;
}
...
$this->codeE('2. This case should not happen it means there is a problem with the steps define for the process : '.$this->_name.', step : '.print_r( self::$_instanceA[$this->_name]->_progressO, true));
}}
...
if('failed'==$oneAjax->status){
$this->_progressO->status=$oneAjax->status;
$oneAjax->text='<span class="label label-warning">'.$oneAjax->text.'</span>';
}
$returnA[]=$oneAjax;
}}
if($this->_progressO->status=='failed'){
$obj=new WProgress_return;
$obj->id='WAjxStatus';$obj->text='<span class="label label-danger"><i class="fa fa-exclamation"></i>'.WText::t('1242282449PIPX'). '</span>';
$obj->status='failed';
$returnA[]=$obj;
}elseif($this->_progressO->ratio >=100){
$obj=new WProgress_return;
...
if(!empty($this->_estimateLeft))$this->_progressHTML->durationValue=WTools::durationToString($this->_estimateLeft * 1.2 );
...
var $maxIncr=0;
var $ratio=0;
var $orign=0;var $start=0;var $finish=0;
var $params=null;
var $isSchedule=false;
}
class WProgress_return {
var $status='success'; var $id='';
var $action='html';var $text='';
}