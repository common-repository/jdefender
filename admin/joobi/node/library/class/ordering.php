...
if(empty($data) || 1==count($data)) return $data;
if(false && isset($parent['depth'])){
$parentOrderA=array();
$maxDepth=10;
...
$dpt=$parent['depth'];
if($d->$dpt !=$i ) continue;
$ord=self::_getOrderTree($d, $count, $parent['parent'], $parent['depth'], $parentOrderA, $maxDepth );
$pkMap=$parent['pkey'];
...
private static function _getOrderTree($d,&$count,$prtMap,$dpthMap,$parentOrderA,$maxDepth){
...
$ord=self::_checkOrdering($ord, $orderingA );
$orderingA[$ord]=true;
return $ord;
}
private static function _checkOrdering($ord,$orderingA){
if(!isset($orderingA[$ord])) return $ord;
$ord++;
return self::_checkOrdering($ord, $orderingA );
}
}