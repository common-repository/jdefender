...
if(!isset($_SESSION[$domain]) || !is_object($_SESSION[$domain]))$_SESSION[$domain]=new stdClass;
...
$sls=( defined('JOOBI_SSL_USED')?JOOBI_SSL_USED : false);
if( IS_ADMIN && ! $sls){$secure=false;
}elseif(!IS_ADMIN && ! $sls){$secure=false;
}}}
...
public static function setfile($id,$data,$location='',$delete=false){
$idpath=(string)$id;
if(!empty($location)){
$location=rtrim($location, '/') .'/';
}$path=JOOBI_DS_USER.'progress/'.$id.'.data';
if($delete){
if( is_file($path )) return @unlink($path );
else return false;
}
if(!is_file( JOOBI_DS_USER.'progress')){
@mkdir( JOOBI_DS_USER.'progress');
}
return @file_put_contents($path, serialize($data));
}  
public static function getfile($id,$data='',$location=''){
$idpath=(string)$id;
if(!empty($location)){
$location=rtrim($location, '/') .'/';
}$path=JOOBI_DS_USER.'progress/'.$id.'.data';
...
if(empty($extension) || 'zzzzz'==$extension){
$getApp=WApplication::getApp();
if('jvendor'==$getApp)$getApp='jmarket';
$extension=WExtension::get($getApp.'.application','wid');
}
...
return (bool)$data;
case 'safejs':$p=array( "'", '"', "\n\r", "\r\n", "\n", "\r" );
return (string)str_replace($p, ' ',$data );
case 'phrase':
...
case 'string':
  ...
  $data=str_replace( array('____','___','__') , '_',$data );
...
$cacheFolder=$mainFolder.'/HTMLPurifier';
