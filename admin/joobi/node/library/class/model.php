...
var $_keepAttributesOnDelete=false;var $_keepAttributesOnSave=false; 
var $_doNotDeleteValidate=false;
var $_getChildIds=false;
var $_checkModel=false; 
var $_pubProperties=null;
var $_validate=true;
var $_singleDelete=false;
var $_noCoreCheck=false;
var $_optimizeImg=true;
private $_skipMessage=false;
var $_saveOrder=false;
var $_fileInfo=array();
public static $safeModeModelElementsA=array();
public static $modelReady=null;
protected $_loadElementBeforeDelete=false;
var $_checkNamekey=true;
...
$tempdata=self::loadDataModel($path, false, true, $showMessage );
}else{
...
if(!empty($tempdata->type) && $tempdata->type==40 )  WLoadFile('category.model.node', JOOBI_DS_NODE );
...
if($myPos===false){
if(!empty($tempdata->type) && $tempdata->type==20){ $myFuntion=substr($tempdata->namekey, 0, -5 );
$myFile='trans';
...
if(!empty($tempdata->type) && $tempdata->type==40)$newClass=new Category_Node_model($params );
else {
$newClass=new $modelClassName($params );
}
...
if(!empty($this->_infos->namekey)){
$namekey=$this->_infos->namekey;
$this->codeE('The model '.$namekey.' does not support data caching.');
}else{
WMessage::log( @$this->_infos , 'model-not-define-error');
}return false;
}
...
$DATE=$property;
WMessage::log(  'The date ('.$DATE.') has been cancelled because it is in the past!','validateDate_date_in_past_error');
WMessage::log(  $this->$property , 'validateDate_date_in_past_error');
WMessage::log(  $this , 'validateDate_date_in_past_error');
$this->$property=0;
...
$tbID=$MyModel->getTableID();
if(empty($tbID )){
continue;
}
$tablesIds[ $tbID ]=$tbID;
$modelTable[ $tbID ]=$mySid;
}}
if( count($tablesIds) < 2 ) return true;
$myKey=serialize($tablesIds );
if(!isset($FKresultA[$myKey])){
if( WGet::maintenanceMode()){
$FKresultA[$myKey]=array();
foreach($tablesIds as $dbtid){
$key='foreignsA|'.$dbtid;
if(!empty(self::$safeModeModelElementsA[$key])){
foreach( self::$safeModeModelElementsA[$key] as $fkO){
if($fkO['dbtid']==$fkO['ref_dbtid'] ) continue;
if( in_array($fkO['dbtid'], $tablesIds ) && in_array($fkO['ref_dbtid'], $tablesIds )){
$objO=new stdClass;
$objO->dbtid=$fkO['dbtid'];
$objO->ref_dbtid=$fkO['ref_dbtid'];
$objO->name=$fkO['name'];
$objO->name2=$fkO['name2'];
$FKresultA[$myKey][]=$objO;
}}
}
}}else{
$caching=( defined('PLIBRARY_NODE_CACHING')?PLIBRARY_NODE_CACHING : 1 );
...
public static function loadDataTable($path,$insertDB=false){static $tempdataA=array();
static $dbtidA=array();
if( is_numeric($path)){
if(isset($dbtidA[$path]) && isset($tempdataA[$dbtidA[$path]]))$tempdata=$tempdataA[$dbtidA[$path]];
else $tempdata=null;
}elseif(isset($tempdataA[$path])){
$tempdata=$tempdataA[$path];
}else{
$table=str_replace('.','_', substr($path, 1 ));
$tableLd=str_replace('#','.',$table );
$tempdata=WGet::loadData($tableLd, 'table');
if(!empty($tempdata)){
$possbleChildA=array('columnsA','foreignsA','contraintsA');
foreach($possbleChildA as $child){
if(!empty($tempdata->$child)){
$key=$child.'|'.$tempdata->dbtid;
self::$safeModeModelElementsA[$key]=$tempdata->$child;
}
}
if($insertDB){
$libraryReaddata=WClass::get('library.readdata');
$libraryReaddata->populateTable($tempdata );
}
$tempdataA[$path]=$tempdata;
$dbtidA[$tempdata->dbtid]=$path;
}
}
return $tempdata;
}
public static function loadDataModel($path,$insertDB=false,$convertPrams=true,$showMessage=true){static $tempdataA=array();
static $sidA=array();
if( is_numeric($path)){
if(isset($sidA[$path]) && isset($tempdataA[$sidA[$path]]))$tempdata=$tempdataA[$sidA[$path]];
else {
$controller=WGlobals::get('controller');
$controllerA=explode('-',$controller );
$controller=array_shift($controllerA );
$folderS=WGet::folder();
$folderS->displayMessage();
$modelsA=$folderS->files( JOOBI_DS_NODE . $controller.'/data/model');
if(!empty($modelsA)){
foreach($modelsA as $file){
if('index.html'==$file ) continue;
$file=str_replace('_node.php','.php',$file );
$model=str_replace('_','.', substr($file, 0, -4 ));
$tempdata=WModel::loadDataModel($model, false, false);
if($path==$tempdata->sid){
break;
}else{
$tempdata=null;
}
...
$trans='';
if( strpos($path, '.')===false && 'trans'==substr($path, -5 )){
$path2=substr($path, 0, -5 );
$trans='trans';
}else{
$path2=$path;
}
$pathA=explode('.',$path2 );
$node=array_shift($pathA );
if(empty($pathA)){
if(empty($trans))$pathLD=$path2.'_node';
else $pathLD=$path2.'_'.$trans;
}else{
$pathLD=$path2;
$pathLD=str_replace('.','_',$pathLD );
}
$tempdata=WGet::loadData($node.'.'.$pathLD, 'model','', false, $showMessage );
if(!empty($tempdata)){
if(!is_numeric($tempdata->rolid))$tempdata->rolid=WRole::get( substr($tempdata->rolid, 1 ), 'rolid');
$tempTable=self::loadDataTable($tempdata->dbtid, $insertDB );
if(!empty($tempTable)){
$tempdata->tablename=$tempTable->name;
$tempdata->tableprefix='';
$tempdata->tablegroup=$tempTable->group;
$tempdata->pkey=$tempTable->pkey;
$tempdata->type=$tempTable->type;
$tempdata->dbid=$tempTable->domain;
$tempdata->dbtidSAFE=$tempTable->dbtid;
$tempdata->domain=$tempTable->domain;
$tempdata->export=$tempTable->export;
if($convertPrams && !empty($tempdata->params)){
WTools::getParams($tempdata, 'params');
}
$sid=$tempdata->sid;
if(empty($tempdata->mainmodel)){
...
if(empty($tempdata->addon))$tempdata->addon=WGet::DBType(); 
if(!empty($tempdata->params)){
$myParams=$tempdata->params;
WTools::getParams($tempdata, 'params');
$tempdata->params=$myParams;
}
$tempdata->pkey=trim($tempdata->pkey );
$allPK=explode(',',$tempdata->pkey );
$tempdata->primaryKeys=$allPK;
...
WLoadFile($localtion, JOOBI_LIB_CORE.'model/');
...
$libraryForeignM->select('map', 1, 'name');
$libraryForeignM->select('map2', 2, 'name2');
