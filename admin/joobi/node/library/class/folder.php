...
public function create($FOLDER='',$base=null,$safe=true,$chmod='0755'){
 ...
 if(!$this->create($fold, $base, $safe, $chmod ))$status=false;
  ...
  if(!empty($base)){
 $base=rtrim($base, '/');
 $FOLDER=$base.'/'.rtrim($FOLDER, '/');
 }else{
 $FOLDER=rtrim($FOLDER, '/');
 }
if(empty($FOLDER)) return false;
if($this->exist($FOLDER)){
if($safe ) return self::$hddFileC[$this->system]->safe($FOLDER );
return true;
}
...
return self::$hddFileC[$this->system]->checkExist();
 ...
 if(!empty($base))$base=rtrim($base, '/');
$foler=($base !=''?$base.'/' : ''). $folder;
 $base=rtrim($base, '/');
...
if($full ) foreach($resultA as $k=> $p)$resultA[$k]=$path.'/'.$p;
return $resultA;
}
if($this->message){
$mess=WMessage::get();
...
$path=$folder.'/'.$exc;
if($this->exist($path)){
array_walk($list, array('Library_Folder_class','_match'), rtrim($exc,DS).'/');
...
if(empty($folder))$folder=rtrim( JOOBI_DS_ROOT, '/');
if(!$this->exist($folder)){
if($this->message){
$mess=WMessage::get();
...
return self::$hddFileC[$this->system]->totalSpace($folder);
 ...
 public function tempFolder($action='create',$i=0,$safe=true){
 switch($action){
 case 'create':
 case 'generate':
 $returnpath=false;
...
$status=$this->create($tempfolder, '',$safe );
if($status){
if($returnpath){
return $tempfolder;
}return $current;
...
break;
 ...
 $folderA=$this->folders($folder );
 if(!empty($folderA)){
 foreach($folderA as $fld){
 $this->delete($folder.'/'.$fld );
 } }
 $filesA=$this->files($folder );
 if(!empty($filesA)){
 $fileS=WGet::file($this->system );
 foreach($filesA as $fls){
 $fileS->delete($folder.'/'.$fls );
 } } 
 }
...
$dst=rtrim($dst, '/');
 ...
 if(!empty($base)){
 $base=rtrim($base, '/');
 $src=(!empty($base)?$base.'/' : ''). rtrim($src, '/');
 $dst=(!empty($base)?$base.'/' : ''). $dst;
 }
 $src=rtrim($src, '/');
if(!$this->exist($src)){
if($this->message){
