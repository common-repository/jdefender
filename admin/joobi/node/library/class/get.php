...
if(empty($expiration))$expiration=WTools::timeOut();$sessionPreferences=new stdClass;
...
if(!defined('JOOBI_SSL_USED')){
if( WRoles::isAdmin('manager')){
if( is_bool( JOOBI_SSL_BE )){
define('JOOBI_SSL_USED', JOOBI_SSL_BE );
}else{
define('JOOBI_SSL_USED', WPref::load('PLIBRARY_NODE_SSLBE'));
}}else{
if( is_bool( JOOBI_SSL_FE )){
define('JOOBI_SSL_USED', JOOBI_SSL_FE );
}else{
define('JOOBI_SSL_USED', WPref::load('PLIBRARY_NODE_SSLFE'));
}}}
switch($extType){
...
$w=WExtension::get( WApplication::getApp(). '.application','wid');
...
if( WGet::readConfig('install') && WRoles::isAdmin('manager') && WExtension::exist('main.node') && WApplication::isEnabled( JOOBI_MAIN_APP )){
$myCtrl=WGlobals::get('task');
if('installnow' !=$myCtrl){
$mess=WMessage::get();
$mess->adminE('It seems that the installation was interupted. Please finish the installation in order for the system to function properly!');
}}
break;
case 'module': 
self::_filterGet();
...
WHook::triggerAction('onAppRender');
...
static $status=null;
if(!isset($status)){
if( WGet::readConfig('maintenance'))$status=true;
if( WGet::readConfig('install'))$status=true;
}
return $status;
}
public static function readConfig($name=null,$default=null){
static $config=null;
if(!isset($config)){
...
if( class_exists('Joobi_Config')){
if('installnow' !=WGlobals::get('task')){
echo 'Library: Joobi Configuration file could not be loaded...';
exit;
}else{
$config=new Joobi_Config;
$config->dbHost='';
$config->dbName='';
$config->dbUser='';
$config->dbPwd='';
...
if( JOOBI_CONFIG_SAVE
&& ($config->dbHost !=JOOBI_DB_HOSTNAME
|| $config->dbName !=JOOBI_DB_NAME  
|| $config->dbUser !=JOOBI_DB_USER 
|| $config->dbPwd !=JOOBI_DB_PASS 
|| $config->dbPrefix !=JOOBI_DB_PREFIX 
|| $config->dbType !=JOOBI_DB_TYPE 
|| $config->sitePath !=JOOBI_SITE_PATH
|| $config->siteURL !=JOOBI_SITE
)
){
$data=array();
$data['dbHost']=JOOBI_DB_HOSTNAME;
$data['dbName']=JOOBI_DB_NAME;
$data['dbUser']=JOOBI_DB_USER;
$data['dbPwd']=JOOBI_DB_PASS;
$data['dbPrefix']=JOOBI_DB_PREFIX;
$data['dbType']=JOOBI_DB_TYPE;
$data['sitePath']=JOOBI_SITE_PATH;
$data['siteURL']=JOOBI_SITE;
$data['index']=JOOBI_INDEX;
$data['ajaxURL']=URL_NO_FRAMEWORK;
$data['sessionTime']=JOOBI_SESSION_LIFETIME;
$data['listLimit']=JOOBI_LIST_LIMIT;
$data['frameworkDebug']=JOOBI_DEBUGCMS;
if(empty($config->secret ) || 'JoobiDev'==$config->secret){
$data['secret']=WTools::randomString( 87, true);
...
$config->dbPrefix=JOOBI_DB_PREFIX;
$config->dbType=JOOBI_DB_TYPE;
$config->sessionTime=JOOBI_SESSION_LIFETIME;
$config->listLimit=JOOBI_LIST_LIMIT;
$config->frameworkDebug=JOOBI_DEBUGCMS;
$config->sitePath=JOOBI_SITE_PATH;
$config->siteURL=JOOBI_SITE;
$config->index=JOOBI_INDEX;
$config->ajaxURL=URL_NO_FRAMEWORK;
$libraryConfigC=WClass::get('library.config');
$libraryConfigC->updateConfigFile($config, $data );
}
if('joomla'=='JOOBI_FRAMEWORK_TYPE' && ! IS_ADMIN)$config->install=false;
if($config->maintenance && $config->maintAdmin && ! IS_ADMIN){
$config->maintenance=false;
}if($config->install && $config->maintAdmin && ! IS_ADMIN){
...
public static function loadData($namkey,$type,$property='',$reset=false,$showMessage=true){
static $dataA=array();
if($reset){
$dataA=array();
return true;
}
if(empty($namkey ) || is_numeric($namkey)){
return false;
}
$key=$type.'|'.$namkey;
if(!isset($dataA[$key])){
$namkeyA=explode('.',$namkey );
$ct=count($namkeyA);
if($ct==2){
$classA=$namkeyA;
$node=array_shift($namkeyA );
$path=implode('.',$namkeyA );
$exit=WLoadFile($path, JOOBI_DS_NODE . $node.'/data/'.$type, true, false);
}else{
$classA=$namkeyA;$path=array_pop($namkeyA );
$node=array_pop($namkeyA );$theme=array_pop($namkeyA );
$exit=WLoadFile($path, JOOBI_DS_USER.'theme/'.$node.'/'.$theme.'/data/'.$type, true, false);
}
if(!$exit){
$dataA[$key]=false;
if($showMessage){
WMessage::log($node.'/data/'.$type.'/'.$path, 'data-file-not-found');
WMessage::log($node.'/data/'.$type.'/'.$path , 'data-file-not-found-CHECK_THAT');
WMessage::log( debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS ) , 'data-file-not-found-CHECK_THAT');
}
return false;
}
$class='Data_';
...
WMessage::log('class not found: '.$class , 'data-file-not-found');
$dataA[$key]=false;
return false;
}
}else{
if(empty($dataA[$key])) return false;
}
if(!empty($dataA[$key])){
if(empty($property)){
return $dataA[$key];
}else{
if(isset($dataA[$key]->$property)){
return $dataA[$key]->$property;
}}}
$dataA[$key]=false;
if($showMessage ) WMessage::log('Other error for the type: '.$type.' and the ID : '.$namkey , 'data-file-not-found');
return false;
}
public static function DBType(){
if(!defined('PLIBRARY_NODE_DBTYPE')){
return ( WGet::maintenanceMode()?'mysqli' : JOOBI_DB_TYPE );
}else{
...
return ( WGet::maintenanceMode()?'mysqli' : $dbtype );
}}
}
}
...
$maintenance=WGet::maintenanceMode();
if($maintenance){
static $widA=array();
static $tempdataA=array();
if( is_numeric($path)){
if(isset($widA[$path])){
$tempdata=$tempdataA[$widA[$path]];
}else{
}}else{
if(isset($tempdataA[$path])){
$tempdata=$tempdataA[$path];
}}
if(empty($tempdata)){
$pathA=explode('.',$path );
if( count($pathA) > 2){
$pathExt=$pathA[0].'.'.$pathA[1].'_'.$pathA[2];
}else{
$pathExt=$path;
}
$tempdata=WGet::loadData($pathExt, 'extension','', false, $showMessage );
if(!empty($tempdata)){
if(!empty($tempdata->trans)){
$folder=$pathA[0];
if(!empty($loadedLang[$folder])) return true;
$vocabExist=WGet::loadData($folder.'.vocab','translation');
if($vocabExist && !empty($vocabExist->translationA)){
WText::$vocab=array_merge( WText::$vocab, $vocabExist->translationA );
}
}
$tempdataA[$tempdata->namekey]=$tempdata;
$widA[$tempdata->wid]=$tempdata->namekey;
}else{
$tempdata=null;
if($showMessage){
WMessage::log('Extension namekey: '.$path.'  -  property: '.$property, 'extension-not-exist');
WMessage::log( debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS ), 'extension-not-exist');
}}
}
}else{
...
$wid=$tempdata->wid;
if(!$maintenance && ! isset($transloaded[$wid])){
...
if(!WPref::load('PLIBRARY_NODE_SUGAR') || WPref::load('PLIBRARY_NODE_SUGAR') !=$subtype){
$libraryPref=WPref::get('library.node');
$libraryPref->updatePref('sugar',$subtype );
}}
}else{
if( JOOBI_FRAMEWORK_TYPE=='joomla'){
$subtype=WPref::load('PLIBRARY_NODE_SUGAR');
if($subtype > 0 ) WGlobals::setSugar(($subtype !=21 && $subtype !=31?true : false));
}
}
if(!$all){
$appsInfoC=WCLass::get('apps.info');
...
public static function &get($return='object',$uid=null,$loadFromUID='uid'){
...
$roleC=WRole::get();
$instance->rolids=$roleC->getUserRoles($instance->uid, $instance->rolid );
$loadRolesB=false;
}
if( WGet::maintenanceMode()){
$instance->lgid=1;
...
public static function timezone($uid=null,$DST=true){
static $timezone=array();
if(!isset($timezone[$uid.'|'.$DST])){$delay=WUser::get('timezone',$uid ) * 60;
$timezone[$uid.'|'.$DST]=$delay;}
return (int)$timezone[$uid.'|'.$DST];
}
public static function dayLightSaving($uid=null){
$delay=WUser::get('timezone',$uid ) * 60;
return WTools::dayLightSaving($delay );
}
...
if(!WGet::readConfig('install') && WUser::$ready){$extension=WExtension::get($myFuntion .'.node','data', null, null, false);
...
static $loaded=false;
if(!$loaded){
WLoadFile('type' , JOOBI_LIB_CORE.'class/');
$loaded=true;
}
$myPos=strpos($path, '.');
$myFuntion=substr($path, 0 , $myPos );
...
$basePath=(!empty($params->custom)?JOOBI_DS_USER.'custom/' : JOOBI_DS_NODE );
...
WController::ajaxReturnObj('redirect','', WPages::link($url, $wPageID ));
...
$curController=WGlobals::get('controller');
$curTask=WGlobals::get('task');
if($ctr==$curController && $tsk==$curTask)$extraPrev='xv='.rand( 1, 9 );
else $extraPrev='';
}else{
if(!IS_ADMIN)$extraPrev='xv='.rand( 1, 9 );
}
...
$extraPrev=$charUsed . $extraPrev;
if(false !==strpos($url, $charUsed.'xv=')){
$url=str_replace($charUsed.'xv=',$extraPrev, $url );
}else{
$url .=$extraPrev;
}}
if( headers_sent()){
...
if(!defined('JOOBI_URL_THEME_JOOBI')) WView::definePath();
WLoadFile('blueprint.'.$element, JOOBI_DS_NODE.'output/', true, false);
...
if(empty($alreadyDoneA[$type][$loadOn][$name][$script]))$allScriptsA[$type][$loadOn][$name][]=$script;
$alreadyDoneA[$type][$loadOn][$name][$script]=true;
}
return '';
}
...
$tempalte=WPage::getTemplate('path'). '/'.JOOBI_FOLDER.'/'.str_replace('/','/',$tmpFile );
if( file_exists($tempalte )){
return WPage::getTemplate('url'). '/'.JOOBI_FOLDER.'/'.$tmpFile;
}
}}
...
return $path . $file.'?v='.JOOBI_VERSION;
}
...
$secondJSONA=array();
foreach($valueA as $k=> $v ) if(!empty($v))$secondJSONA[]="$k=$v";
...
WPage::includeRootScript();
if(!empty($JSparams['action']))$JSparams['action']=WGlobals::filter($JSparams['action'] , 'safejs');
...
if( is_string($value) || empty($value))$value='\''.$value.'\'';
if($key=='confirm'){
...
if(!empty($key) && !empty($value))$joobiButtonJS .=",'" . $key . "':" . $value;
}
$joobiButtonJS .="};";
...
WPage::includeRootScript();
$path=WPage::fileLocation('js','js/validation.1.2.js');
...
if(!is_numeric($wid)){
if( WGet::maintenanceMode()){
if(isset($alreadyA[$wid])) return;
$alreadyA[$wid]=true;
$widA=explode('.',$wid );
array_pop($widA );
$vocabExist=WGet::loadData( implode('.',$widA ). '.vocab','translation');
if($vocabExist && !empty($vocabExist->translationA)){
WText::$vocab=array_merge( WText::$vocab, $vocabExist->translationA );
}
return true;
...
class WHook {
public static $_hooksA=array();
public static function addAction($action,$classToCall,$functionToCall,$argument=null,$priority=10,$type='class'){
if(empty($action) || empty($classToCall) || empty($functionToCall)) return false;
self::$_hooksA[$action][$priority][]=array('c'=> $classToCall, 'f'=> $functionToCall, 'a'=> $argument, 't'=> $type );
}
public static function triggerAction($action){
if(!isset( self::$_hooksA[$action] )) return false;
foreach( self::$_hooksA[$action] as $priority=> $myHookA){
foreach($myHookA as $actionA){
if('class'==$actionA['t']){
$classC=WClass::get($actionA['c'] );
if(!empty($classC)){
$fct=$actionA['f'];
if( method_exists($classC, $fct )){
$classC->$fct($actionA['a'] );
}}}
}
}
unset( self::$_hooksA[$action] );
}
