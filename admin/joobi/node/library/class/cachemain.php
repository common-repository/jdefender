...
$this->_now=time();
$this->_root=WApplication::cacheFolder(). '/';
}
...
public function clean($folder){
$return=true;
if(!isset(self::$_fileS)) self::$_fileS=WGet::folder();
self::$_fileS->displayMessage();
if(empty($folder)){
$return=self::$_fileS->delete($this->_root );
if($return)$return=self::$_fileS->create($this->_root );
}else{
if( is_string($folder))$return=self::$_fileS->delete($this->_root.'/'.$folder );
elseif(is_array($folder)) foreach($folder as $f)$return=self::$_fileS->delete($this->_root.'/'.$f );
}
return $return;
}
