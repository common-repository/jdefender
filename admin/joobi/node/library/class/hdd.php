...
return @is_dir($path);
 ...
 return JOOBI_DS_USER . $folder . (!empty($path)?'/'.$path : '');
 }
...
if(is_dir($path. '/'.$name)){
if(!$this->delete($path. '/'.$name)) return false;
}else{
if(!$this->delete($path. '/'.$name,true)) return false;
}
}
closedir($dirHandle);
@chdir($old_cwd);
...
if($recursive){
if(!empty($baseFolder))$baseFolder=$baseFolder.'/'.$name;
else $baseFolder=$name;
if(is_array($path)){
$result=$this->content( array($path[0],$path[1]. '/'.$name), $filter, $recursive, $full, $type, $excludeA, $baseFolder );
}else{
...
if(!empty($result) && !empty($name) && empty($excludeA)){
  $newA=array();
  foreach($result as $k=> $p)$newA[$k]=$name.'/'.$p;
  $result=$newA;
}
if(!empty($excludeA)){
$newA=array();
...
if($backup && $this->exist($dst.'/'.$fold )) continue;
if($this->create($dst.'/'.$fold, '0755', false, true)===false){
return false;
}}}
...
$backup_file=str_replace('/'.'/','/', rtrim( JOOBI_DS_USER.'backup','/'). '/'.ltrim( substr($dst.'/'.$myfile, strlen(JOOBI_DS_ROOT)), '/'));
...
if(!$this->move($dst.'/'.$myfile, $backup_file )){
$FILE=$dst_file;
...
$this->changeAccess($path, $chmod );
  ...
  if(empty($folder))$folder=$this->folder;
