...
WMessage::log( debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS ), 'failed-cancelQuery'); 
return false;
}
...
if(! isset($this->_new)){
if($this->multiplePK()){
$expolodePK=$this->getPKs();
if(!empty($expolodePK)){
$this->_new=false;
foreach($expolodePK as $onePK){
if(empty($this->$onePK ))$this->_new=true;
}}else{
$this->_new=true;
}}else{
$pKey=$this->getPK();
$this->_new=(!empty($this->$pKey )?false : true);
}}
return $this->store();
  }
...
if( WTable::$addDBName){
$dbN=$this->getDBName();
if(!empty($dbN))$dbN=$this->_db->$function($dbN ). '.';
$tb=$dbN . $tbName.' ';
} else $tb=$tbName.' ';
if($this->_identify){
...
$whereA=$this->_makeWhere();
if(empty($whereA)){
WMessage::log('Attempt to delete all data from the database with where','delete_all_error');
WMessage::log( debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS ), 'delete_all_error');
return false;
}
$this->_query .=$whereA . $qLimit; return $this->setQuery('',$this->_query );
}else{
...
WMessage::log('Attempt to delete all data from the database with Ids','delete_all_error');
WMessage::log( debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS ), 'delete_all_error');
}
$mess=WMessage::get();
...
if(isset($this->_infos->pkey)){
$pos=strpos($this->_infos->pkey, ',');
if(false !==$pos){
return substr($this->_infos->pkey, 0, $pos );
}else{
return $this->_infos->pkey;
}}
return null;
}
public function getPKs(){
if(isset($this->_infos->primaryKeys)){
return $this->_infos->primaryKeys;
}else{
if(!empty($this->_infos->pkey)){
return explode(',',$this->_infos->pkey );
}else{
return null;
}}}
...
WMessage::log('PKs not defined','PK_not_defined');
WMessage::log($this , 'PK_not_defined');
}}
return $myStatus;
  }
...
$result=$this->showColumns($this->makeT());
if(empty($result)) return false;
$hasColumn=false;
foreach($result as $c=> $r){
if($c==$columnName){
$hasColumn=true;
break;
}}
if($hasColumn ) return $this->load('q',$this->_db->dropColumn($this->makeT(), $columnName ));
  else return false;
}
...
if(!WGet::maintenanceMode()) return (isset($this->_infos->dbtid)?$this->_infos->dbtid : 0 );
else return (isset($this->_infos->dbtidSAFE)?$this->_infos->dbtidSAFE : 0 );
}
...
if($useMultipleLang && !empty($this->lgid) && 20==$this->getType()){
