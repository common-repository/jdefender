...
if( WGet::readConfig('install') || (( JOOBI_DEBUGCMS || WPref::load('PLIBRARY_NODE_DBGERR')) && WRole::hasRole('manager'))){$this->addM($mess, $type, $variable, true);
}else{
...
$this->log($mess, 'admin_error_messages');
return;
}
if(!WGet::readConfig('install')){$this->log($mess, 'error_messages');
}elseif(!IS_ADMIN){
$this->log($mess, 'error_messages');
return '';
}
if(!is_string($mess)){
$this->log($mess, 'should-be-a-string-message');
}
$key=$type .'|'. md5($mess ); if(!isset($message[$key])){
...
if( is_string($message) && is_numeric($message[0]))$message=WText::t($message );
}
...
if(!WGet::maintenanceMode() && ( WExtension::exist('main.node'))){$vocabulary=self::loadVocab();
$allVocab=array();
if(!empty($vocabulary) && is_array($vocabulary)){
...
$model->rememberQuery(true, 'Translation');
...
return $mess;
$fileName=str_replace( JOOBI_DS_ROOT, '/',$backtraceItem['file'] );
if($html){
$mess .='<br/>';
}else{
$mess .="\r\n";
...
$details='';
if($showTime){
$details .=' date: '.WApplication::date( WTools::dateFormat('date-time-second', true)). ' ';
}
...
$writeStatus=$filehandler->write($file, $message . "\r\n", 'append');
return $writeStatus;
}
...
case 'filesystem':
default:
return JOOBI_DS_USER.'logs/'.$file;
}}
