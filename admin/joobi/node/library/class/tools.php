...
if(empty($maxExecutionTime) || $maxExecutionTime < 1)$maxExecutionTime=30;
...
$lifetime=WTools::timeOut();
if($lifetime < 3600)$lifetime=3600;if($lifetime > 604800)$lifetime=604800;
...
$sessionTime=WPref::load('PLIBRARY_NODE_SESSIONTIME');
if($sessionTime < JOOBI_SESSION_LIFETIME)$sessionTime=JOOBI_SESSION_LIFETIME;
if($sessionTime < 5)$sessionTime=5;
$lifetime=$sessionTime * 60; 
return $lifetime;
}
public static function dayLightSaving($offset){
static $lightA=array();
if(isset($lightA[$offset])) return $lightA[$offset];
$zoneName='';
$allZoneA=timezone_abbreviations_list();
foreach($allZoneA as $contA){
foreach($contA as $timeZoneA){
if($offset==$timeZoneA['offset']){
$zoneName=$timeZoneA['timezone_id'];
break;
}}if(!empty($zoneName)) break;
}
$date=new DateTime('now '.$zoneName );
if(!empty($date))$lightA[$offset]=($date->format('I') * 3600 );
return (!empty($lightA[$offset])?$lightA[$offset] : 0 );
}
...
public static function checkMemory($allocate=4000000,$available=false,$safePercent=0){
static $maxMemory=0;
if(empty($maxMemory )){
$maxMemory=@ini_get('memory_limit');
if(empty($maxMemory ))$maxMemory=@get_cfg_var('memory_limit');
...
$peakMemory=memory_get_usage() * 1.4; }else{
$peakMemory=memory_get_peak_usage();
}
if($safePercent > 1){
$peakMemory=$peakMemory * ( 1 + $safePercent / 100 );
}
if($peakMemory > $maxMemory ) return false;
if(empty($allocate))$allocate=4000000;
