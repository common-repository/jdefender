...
$allDomainsA=( ! is_array($domain )?array($domain) : $domain );
$registry=$this->_getRegistry($type );
unset($_SESSION['extInfo'] );
foreach($allDomainsA as $oneDom){
if($oneDom=='Preference') unset($_SESSION['jPreference']['uid'] );
$status=$registry->clean($oneDom );
}
return $status;
}
}else{
