...
if(!$hasDefault){
$localtion=JOOBI_DS_NODE . $folder.'/preferences.php';
...
$localtion=JOOBI_DS_USER.'node/'.$folder.'/preferences.php';
if( @file_exists($localtion)){
include($localtion);
}else{
...
$username='u'.$uid;
$localtion=JOOBI_DS_USERS . $username.'/node/'.$folder.'/preferences.php';
if( @file_exists($localtion)){
include($localtion);
}else{
$needUserClass=true;
...
if($needUserClass){
$className='User_'.$uid.'_'.$folder.'_preferences';
if(!class_exists($className)){
$siteCl='Site_'.$folder.'_preferences';
if(!class_exists($siteCl)){
$userClass='class User_'.$uid.'_'.$folder.'_preferences extends Default_'.$folder.'_preferences{}';
}else{
$userClass='class User_'.$uid.'_'.$folder.'_preferences extends Site_'.$folder.'_preferences{}';
}
eval($userClass);
}
}else{
...
if(empty($accessInst->$key) || ! WRole::hasRole($accessInst->$key)){
return false;
}}
if('premium'==$keyName){
return false;
