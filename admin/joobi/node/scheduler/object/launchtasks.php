...
$cronNxt=WPref::load('PLIBRARY_NODE_NEXTCRON');
if($cronNxt > $time){
$message->userN('1298350272MURB');
$DATE=WApplication::date( WTools::dateFormat('day-date-time') , $cronNxt + WUser::timezone());
...
$cronFrq=WPref::load('PSCHEDULER_NODE_CRONFREQUENCY');
...
if($countpcsid <=intval( WPref::load('PSCHEDULER_NODE_MAXPROCESS'))){
 $tasklimit=intval( WPref::load('PSCHEDULER_NODE_MAXTASKS')) - $countpcsid;
 $schedulerM=WModel::get('scheduler','object');
...
$whereCondition=time() - $maxtime;
$schedProcessM=WModel::get('scheduler.processes');
$schedProcessM->where('created','<=',$whereCondition );
...
if(!$statusQuery){
$schedulerProcesssT=WTable::get('scheduler_processes','','pcsid');
...
$pcsid='&pcsid='.$pcsid;
$urllink=JOOBI_SITE . WPage::linkNoSEF('controller=scheduler&task=parallel&schid='.$task->schid . $pcsid . $password, 'popup');
