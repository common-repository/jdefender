...
$rolemodelmembers=WTable::get('role_members','main_userdata','rolid,uid');
...
$memberM=WTable::get('members_node','main_userdata','uid');
...
$USERID=$uid;
$message->codeE('The User does not have any role define in the members_node table for the user: '.$USERID.'. It should not be this way.');
$rolids[]=1;
}
...
$roleM=WTable::get('role_node','main_userdata','rolid');
$roleM->makeLJ('role_node','main_userdata','lft','lft', 0, 1, $sign );
...
if($onlyMainRole){ 
$rolidsA=array( WUser::get('rolid',$uid ));
} else $rolidsA=$this->getUserRoles($uid );
...
$membersModel=WTable::get('role_members','main_userdata','rolid,uid');
...
$membersModel->makeLJ('members_node','main_userdata','uid','uid');
$membersModel->select($maps,1);
$uidsFromURolesA=$membersModel->load('ol');
}
$userM=WTable::get('members_node','main_userdata','uid');
...
if( WUser::$ready)$lgid=WUser::get('lgid');
else $lgid=1;
...
$roleM=WTable::get('role_node','main_userdata','rolid');
$roleM->remember('compare'.$rolid1.'-'.$rolid2, true, 'Model_role_node');
if(is_numeric($rolid1)){
$roleM->whereE('rolid',$rolid1);
}else{
$roleM->whereE('namekey',$rolid1);
}
if(is_numeric($rolid2)){
$roleM->whereE('rolid',$rolid2,0,null,0,0,1);
}else{
$roleM->whereE('namekey',$rolid2,0,null,0,0,1);
}
if($rolid1===$rolid2 ) return true;
$roleM->setDistinct();
$roleM->setLimit( 1000 );
$myRoles=$roleM->load('ol',array('rolid','namekey','lft','rgt'));
...
$roleM=WTable::get('role_node','main_userdata','rolid');
$roleM->makeLJ('role_node','main_userdata','lft','lft',0,1,'>=');
