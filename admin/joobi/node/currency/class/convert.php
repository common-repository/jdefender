...
public function currencyConvert($cur1,$cur2,$amount,$reverseConversion=false,$addFees=true) {
		...
		if ( ! WPref::load( 'PCURRENCY_NODE_MULTICUREDIT' ) && ! WPref::load( 'PCURRENCY_NODE_MULTICUR' ) ) return $amount;	
...
WMessage::log( 'One of the currency is not defined! Check your code! function currencyConvert().', 'currency-not-defined' );
WMessage::log( 'Currency 1: ' . $cur1 , 'currency-not-defined' );	WMessage::log( 'Currency 2: ' . $cur2 , 'currency-not-defined' );
WMessage::log( print_r( debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS ), true ) , 'currency-not-defined' );
...
WMessage::log( 'Currency 1: ' . $cur1 . ' - Code: ' . $ORIGIN_CURRENCY . ' - Currency 2: ' . $cur2 . ' - Code: ' . $CONVERTED_CURRENCY , 'currency-no-rate-defined' );
		return WText::t('1341596429OMYB');
	}
	return false;
}
