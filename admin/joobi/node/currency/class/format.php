...
class Currency_Format_class extends WClasses {
	...
	function load($curid,$returnData='object') {
		...
		if ( empty($curid) && !empty( WCart::$currency ) ) {
			$curid = WCart::$currency;
		}		if ( empty($curid) && !empty( WCart::$currency ) ) {
			$curid = WPref::load( 'PCURRENCY_NODE_PREMIUM' );
		}		
		if ( empty($curid) ) {
			$mess = WMessage::get();
			$mess->codeE( 'Currency code not defined in the load' );
			return null;
		}
		if ( !isset($currencyData[$curid]) ) {
			if ( !isset($currencyM) ) $currencyM = WModel::get( 'currency' );
			$currencyM->remember( 'load_' . $curid, true, 'Model_currency_node' );
			if ( is_numeric($curid) ) {
