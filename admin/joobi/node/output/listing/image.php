...
$maxWidth=(int)(!empty($this->element->imgmaxwidth)?$this->element->imgmaxwidth : 50 );
$maxHeight=(int)(!empty($this->element->imgmaxheight)?$this->element->imgmaxheight : 50 );
...
if(!isset($this->data->$modified))$this->data->$modified=time();
}else{
$path='path';
$name='name';
$type='type';
...
$contentImg='';
if($this->element->lid==Output_Listings_class::$indentID){
...
$imageDataO=WPage::newBluePrint('image');
$imageDataO->location=$url.'?t='.$this->data->$modified;$imageDataO->text=$img->$name;
