...
}$uploadPath=JOOBI_DS_TEMP.'uploads/'.$randomKey.'/';
$dirSys=WGet::folder();
...
$r=json_encode( array('name'=>basename($fullPath), 'size'=> filesize($fullPath), 'status'=>'uploaded','info'=> $fileUPdaedText ));
echo $r;
die;
}}  else {
$r=json_encode( array('name'=>basename($axFilesA['name'][$key]), 'size'=>$axFilesA['size'][$key], 'status'=>'error','info'=>$error));
echo $r;
die;
}
}
...
$r=json_encode( array('name'=>basename($fullPath), 'size'=>$currByte, 'status'=>'uploaded','info'=> $fileUPdaedText ));
echo $r;
die;
}
}
