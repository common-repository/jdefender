...
if( is_numeric($this->_did)){
$didUsed=$this->_did;
}else{
$didUsed=WGet::loadData( str_replace('#','.', substr($this->_did, 1 )) , 'picklist','did');
}}else{
$didUsed=$this->_did;
}
...
$defaults=array($didUsed=> $convertValue );
}else{
$defaults=array();
}}else{
$defaults=(is_array($this->value))?$this->value : array($didUsed=> $this->value );
}
...
if(!WGet::maintenanceMode())$this->codeE('The picklist for the field '.$dropdownPL->name.' could not be loaded!');
return false;
...
if(!empty($this->element->selectdid)){
...
}elseif(!empty($this->element->did)){
$this->_did=$this->element->did;
if(!empty($this->element->selectstyle)){
$this->pickListStyle=$this->element->selectstyle;
}else{
