...
$html=$this->_displayCarrousel($data->itemsA, $data->params, 'carrousel',$zoomEffect );
...
$extraClass=($zoomEffect?' class="zoomEffect"' : '');
...
$count=1;
$orignalContainerClass=$tagParams->containerClass;
foreach($productA as $key=> $product){
$tagParams->containerClass=$orignalContainerClass;
if(!empty($tagParams->layoutNbColumn) && 5==$tagParams->layoutNbColumn){
if( 0==($count+4) % 5){
$tagParams->containerClass .=' col-lg-offset-1';
}}
$product->nb=$count;
...
private function _displayCarrousel($productsA,$tagParams,$typeDisplay='carrousel',$zoomEffect=false){
...
$extraClass=($zoomEffect?' zoomEffect' : '');
...
private function _displayAccordionVertical($productsA,$tagParams,$zoomEffect=false){
static $count=0;
if(!empty($productsA)){
...
WLoadFile('blueprint.frame', JOOBI_DS_NODE.'output/');
...
private function _displayAccordionHorizontal($productsA,$tagParams,$zoomEffect=false){
