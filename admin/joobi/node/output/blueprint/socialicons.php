...
self::$_color=$this->value('socialicon.color');
  ...
  if(empty($data->id)){
  $data->id='rdm'.rand(  100000, 999999 );
  }
  $extraHTML='';
  switch($data->type){
  case 'favorite':
...
if(empty($appID )){
$fb='http://joobi.info/facebook-api';
...
if( self::$_showText){
if(!empty($data->id))$html='<div id="'.$data->id.'" class="socialText">'.$html.'</div>';
else $html='<div class="socialText">'.$html.'</div>';
}else{
$html='<div class="socialIcon"><div id="'.$data->id.'">'.$html.'</div></div>';
}
  }else{
$html=str_replace('id="btn'.$data->id, 'id="'.$data->id, $html );
  }
