...
var $form=true;
var $_extraContent=''; 
var $content=null;var $formObj=null;
var $firstFormName=null;
...
protected $formDirection='';
public static $removedElementsA=array();
public static $changeElementsA=array();
public static $removedConditionsA=array();
public static $removedPicklistValuesA=array();
public static $safeModeViewElementsA=array();
...
$this->formDirection='form-'.$this->fdirection;
}elseif(!empty($this->formDirection )){
self::$directionForm[$this->yid]=$this->formDirection;
}else{
self::$directionForm[$this->yid]='';
}
if('form-basic'==$this->formDirection){
$this->formDirection='';
self::$directionForm[$this->yid]='';
}
if(!empty($this->notitle ) && 5 !=$this->notitle){
...
public static function get($yid,$return='html',$params=null,$controller=null,$showMessage=true,$dontCheckRole=false){static $ctrid=0;
static $access=null;
...
$hasInstance=null;
if(empty($layout->phpfile ) && !empty($layout->pnamekey )){
...
if(!empty($layout->custom))$base=JOOBI_DS_USER.'node/';
else $base=null;
...
$exist=WLoadFile($element, $location, true, true);
if(!$exist){
return false;
}
if($custom){
...
$tempdata=WViews::loadDataView($yid, true, $showMessage );
if(empty($tempdata) && is_numeric($yid)){
...
private static function _processProperty(&$element,$newYID,$allIDsA=array()){
if(!empty($element->yid))$element->yid=$newYID;
if(!empty($element->wid))$element->wid=WExtension::get( substr($element->wid, 1 ), 'wid');
if(!empty($element->sid))$element->sid=WModel::get( substr($element->sid, 1 ), 'sid');
if(!empty($element->rolid)){
$element->rolid=WRole::get( substr($element->rolid, 1 ), 'rolid');
}if(!empty($element->did)){
$didA=explode('#',$element->did );
$element->did=WView::picklist($didA[2], '', null, 'did');
}
if(!empty($element->ref_yid))$element->ref_yid=Wview::get( substr($element->ref_yid, 1 ), 'yid');
if(!empty($element->parentnamekey)){
$nk=substr($element->parentnamekey, 1 );
if(isset($allIDsA[$nk]))$element->parent=$allIDsA[$nk];
}
if(!empty($element->trans)){
$trans2CheckA=array('name','description','wname','wdescription','seotitle','seodescription','seokeywords','textlink','message');
foreach($trans2CheckA as $p){
$TC='TC_'.$p;
if(isset($element->trans->$TC)){
$element->$p=WText::t($element->trans->$TC );
}}}
}
...
if(empty(self::$removedPicklistValuesA[$picklistName])) self::$removedPicklistValuesA[$picklistName]=array();
self::$removedPicklistValuesA[$picklistName]=array_merge( self::$removedPicklistValuesA[$picklistName], $elementListA );
}
public static function getRemovedElements($className=''){
if(isset(self::$removedPicklistValuesA[$className])) return self::$removedPicklistValuesA[$className];
return false;
}
...
self::$changeElementsA[]=$newChangeO;
}
public function removeConditions($elementListA){
if(!is_array($elementListA))$elementListA=array($elementListA );
self::$removedConditionsA=array_merge( self::$removedConditionsA, $elementListA );
}
...
self::$removedElementsA=array();
self::$changeElementsA=array();
self::$removedConditionsA=array();
self::$removedPicklistValuesA=array();
}
...
if(empty(self::$removedElementsA[$parentClass])) self::$removedElementsA[$parentClass]=array();
self::$removedElementsA[$parentClass]=array_merge( self::$removedElementsA[$parentClass], $elementListA );
}
}
...
if(empty( self::$changeElementsA )) return false;
foreach( self::$changeElementsA as $oneChange){
...
if(!empty($this->class))$this->viewClass[]=$this->class;
...
$finalDIV->classes=implode(' ',$this->viewClass ). ' clearfix';
...
if(isset($data->$property) && empty($this->$property))$this->$property=$value;
}
...
if( WGet::readConfig('install') && IS_ADMIN){APIApplication::installThemePath();
return;
}
...
if( IS_ADMIN){
define('JOOBI_URL_THEME_JOOBI', JOOBI_SITE_PATH . JOOBI_FOLDER.'/user/theme/admin/'.$themeFolder.'/');define('JOOBI_DS_THEME_JOOBI', JOOBI_DS_USER.'theme/admin/'.$themeFolder.'/');}else{
$outputSpaceC=WClass::get('output.space');
$spaceO=$outputSpaceC->findSpace();
$themeType=$spaceO->themeTheme;
define('JOOBI_URL_THEME_JOOBI', JOOBI_SITE_PATH . JOOBI_FOLDER.'/user/theme/'.$themeType.'/'.$themeFolder.'/');define('JOOBI_DS_THEME_JOOBI', JOOBI_DS_USER.'theme/'.$themeType.'/'.$themeFolder.'/');}
...
$overwritePath=WPage::getTemplate('path'). '/joobi';
...
public static function loadDataView($yid,$converTrans=true,$showMessage=true){
static $tempdataA=array();
static $yidA=array();
if( is_numeric($yid)){
if(isset($yidA[$yid]) && isset($tempdataA[$yidA[$yid]]))$tempdata=$tempdataA[$yidA[$yid]];
else $tempdata=null;
}elseif(isset($tempdataA[$yid])){
$tempdata=$tempdataA[$yid];
}else{
$orgnYID=$yid;
if( is_string($yid)){
if( substr($yid, 0, 1 )=='#'){
$yidArr=explode('#',$yid );
$yid=$yidArr[1].'.'.$yidArr[2];
}
if( strpos($yid, '.')===false){
$exYIDA=explode('_',$yid );
$yid=$exYIDA[0].'.'.$yid;
}
}
$tempdata=WGet::loadData($yid, 'view','', false, $showMessage );
if(!empty($tempdata)){
...
if(!is_numeric($tempdata->sid)){
$tempdata->sid=WModel::get( substr($tempdata->sid, 1 ), 'sid');
}$tempdata->id=$tempdata->yid;
if($converTrans){
WText::load($tempdata->wid );
if(!empty($tempdata->name))$tempdata->name=WText::t($tempdata->name );
if(!empty($tempdata->description))$tempdata->description=WText::t($tempdata->description );
if(!empty($tempdata->wname))$tempdata->wname=WText::t($tempdata->wname );
if(!empty($tempdata->wdescription))$tempdata->wdescription=WText::t($tempdata->wdescription );
if(!empty($tempdata->seotitle))$tempdata->seotitle=WText::t($tempdata->seotitle );
if(!empty($tempdata->seodescription))$tempdata->seodescription=WText::t($tempdata->seodescription );
if(!empty($tempdata->seokeywords))$tempdata->seokeywords=WText::t($tempdata->seokeywords );
}
$possbleChildA=array('listingsA','formsA','menusA','filtersA','picklistA');
$possbleChildPKA=array('lid','fid','mid');
foreach($possbleChildA as $nb=> $child){
if(!empty($tempdata->$child)){
$newChildA=array();
$parentA=array();
foreach($tempdata->$child as $idx=> $v){
if(!empty($v)){
$obj=new stdClass;
if( is_object($v)){
if(isset($v->publish) && empty($v->publish)) continue;
if(!empty($possbleChildPKA[$nb])){
if(!empty($v->namekey)){
$vvv=$possbleChildPKA[$nb];
$parentA[$v->namekey]=$v->$vvv;
}if(!empty($v->parentnamekey)){
$prt=substr($v->parentnamekey, 1 );
$v->parent=$parentA[$prt];
}}}else{
if(isset($v['publish']) && empty($v['publish'])) continue;
if(!empty($possbleChildPKA[$nb])){
if(!empty($v['namekey'])){
$parentA[$v['namekey']]=$v[$possbleChildPKA[$nb]];
}if(!empty($v['parentnamekey'])){
$prt=substr($v['parentnamekey'], 1 );
$v['parent']=(isset($parentA[$prt])?$parentA[$prt] : 0 );
}}}
foreach($v as $p=> $d){
if(empty($d))$obj->$p=$d;
elseif(  'sid'==$p){
if(!is_numeric($d))$obj->sid=WModel::get( substr($d, 1 ), 'sid');
else $obj->sid=$d;
}elseif(  'ref_sid'==$p){
if(!is_numeric($d))$obj->ref_sid=WModel::get( substr($d, 1 ), 'sid');
else $obj->ref_sid=$d;
}elseif('rolid'==$p){
if(!is_numeric($d))$obj->rolid=WRole::get( substr($d, 1 ), 'rolid');
else $obj->rolid=$d;
}elseif('did'==$p){
if(!is_numeric($d)){
$obj->did=WView::picklist($d, '', null, 'did');
} else $obj->did=$d;
}elseif($converTrans && 'name'==$p)$obj->name=WText::t($d );
elseif($converTrans && 'description'==$p)$obj->description=WText::t($d );
elseif($converTrans && 'textlink'==$p)$obj->textlink=WText::t($d );
else $obj->$p=$d;
}
$obj->yid=$tempdata->yid;
$newChildA[]=$obj;
}
}
$tempdata->$child=$newChildA;
$key=$child.'|'.$tempdata->yid;
WView::$safeModeViewElementsA[$key]=$newChildA;
}
}
$tempdataA[$orgnYID]=$tempdata;
$yidA[$tempdata->yid]=$orgnYID;
}else{
$tempdataA[$yid]=false;
}
}
...
public static function loadDataPicklist($did,$converTrans=true){
static $tempdataA=array();
static $didA=array();
if( is_numeric($did)){
if(isset($didA[$did]) && isset($tempdataA[$didA[$did]]))$tempdata=$tempdataA[$didA[$did]];
else $tempdata=null;
}elseif(isset($tempdataA[$did])){
$tempdata=$tempdataA[$did];
}else{
$orgnID=$did;
if( is_string($did)){
if( substr($did, 0, 1 )=='#'){
$didArr=explode('#',$did );
$did=$didArr[1].'.'.$didArr[2];
}}
if(!WExtension::exist($didArr[1].'.node')) return null;
$tempdata=WGet::loadData($did, 'picklist','', false, true);
if(!empty($tempdata)){
if(!empty($tempdata->wid))$tempdata->wid=WExtension::get( substr($tempdata->wid, 1 ), 'wid');
if(!empty($tempdata->sid))$tempdata->sid=WModel::get( substr($tempdata->sid, 1 ), 'sid');
if(!empty($tempdata->ref_sid))$tempdata->ref_sid=WModel::get( substr($tempdata->ref_sid, 1 ), 'sid');
if(!empty($tempdata->rolid))$tempdata->rolid=WRole::get( substr($tempdata->rolid, 1 ), 'rolid');
if($converTrans){
if(!empty($tempdata->name))$tempdata->name=WText::t($tempdata->name );
if(!empty($tempdata->description))$tempdata->description=WText::t($tempdata->description );
}
$possbleChildA=array('valuesA');
foreach($possbleChildA as $child){
if(!empty($tempdata->$child)){
$newChildA=array();
foreach($tempdata->$child as $idx=> $v){
if(!empty($v)){
...
foreach($v as $p=> $d){
if(empty($d))$obj->$p=$d;
elseif(  'rolid'==$p)$obj->rolid=WRole::get( substr($d, 1 ), 'rolid');
elseif(  'name'==$p)$obj->name=WText::t($d );
else $obj->$p=$d;
}
$newChildA[]=$obj;
}}
$key=$child.'|'.$tempdata->did;
WView::$safeModeViewElementsA[$key]=$newChildA;
}
}
$tempdataA[$orgnID]=$tempdata;
$didA[$tempdata->did]=$orgnID;
}else{
WMessage::log(  'loadDataPicklist( picklist not found : '. $did  , 'picklist_not_found_debug'  );
}
}
return $tempdata;
}
