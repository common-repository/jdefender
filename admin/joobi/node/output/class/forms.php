...
if(!empty($multiform->onlynew) && 1==$multiform->onlynew){
unset($this->elements[$frmKey] );
continue;
}
$typeSplitA=explode('.',$multiform->type );
if(empty($typeSplitA[1])){
...
case 'f[':continue;
break;
case 'c[':case 'u[':case 'v[':if(!empty($multiform->map) && $multiform->sid !=0 && $multiform->typeName !='textonly' && (!isset($multiform->onlynew) || $multiform->onlynew==2))$securityCheckIni[$multiform->sid]['c'][$multiform->map]=true;
$addmeIn=false;
...
&& (empty($multiform->readonly) || !empty($multiform->readonly) && $multiform->typeName=='select')
&& substr($multiform->map, 0, 2 ) !='m['){
...
if(!empty($areaRef ))$this->_area=true;
$securityCheckIni=array();
$exclude=array('x','f','m');foreach($this->elements as $frmKey=> $multiform){
if(!empty($multiform->onlynew) && 2==$multiform->onlynew){
unset($this->elements[$frmKey] );
continue;
}
$typeSplitA=explode('.',$multiform->type );
...
}elseif(!empty($multiform->ldmem) && WUser::get('uid')){
...
$this->removeElementsProcess( self::$removedElementsA, true, 'output_forms_class');
...
if(empty($areaElements )) continue;
$keys=array_keys($areaElements );
if(!empty($areaElements))$this->_renderElements($HTML2, $areaElements, $areaElements, false, $keys );
if(!empty($HTML2[0])){
...
if(!empty($elements))$this->_renderElements($HTML2, $elements, $elements, false, $keys );
if(isset($HTML2[0])){
$HTML=$HTML2[0];
}else{
$HTML='';
...
if(!empty($this->elements))$this->_renderElements($HTML2, $this->elements, $this->elements, false);
if(isset($HTML2[0])){
$HTML=$HTML2[0];
}else{
$HTML='';
...
$multiEID=WGlobals::get($temp_key );
if(is_array($multiEID) && count($multiEID) > 1){
foreach($multiEID as $k=> $oneEID){
$this->formObj->hidden( JOOBI_VAR_DATA . "[$mySID][$pkey][$k]" , $oneEID );
}}else{
...
$this->content .=$HTML;
if(!empty($this->jsvalidtab)){
WPage::addJSScript( "jCore.tabreq=1;" );
}
...
$Temp_map=substr($map, 2, strlen($map)-3 );
$value=WGet::readConfig($Temp_map );
$map=str_replace(']','',$map);
$map=str_replace('[','][',$map);
...
$map=str_replace(']','',$map);
$map=str_replace('[','][',$map);
}elseif($wildPrefix=='f[' || $wildPrefix=='v['){$map=str_replace(']','',$map);
$map=str_replace('[','][',$map);
}else{
if(empty($value)){
$mapComplex=$map.'_'.$element->sid;
...
if(!empty($this->notitle ) && 7==$this->notitle){
$element->spantit=7;
}
...
$checkoutM=WModel::get('checkout','object');
$checkoutM->whereE('sid',  $sid  );
$checkoutM->whereE('eid',  $this->_eid  );
$item=$checkoutM->load('o',array('uid','modified'));
...
if(empty($item)){
$checkoutM->sid=$sid;
$checkoutM->eid=$this->_eid;
$checkoutM->save();
}else{
$checkoutM->setVal('modified', time());
$checkoutM->whereE('sid',$sid );
$checkoutM->whereE('eid',$this->_eid );
$checkoutM->update();
}
...
var $map='';var $value=0;var $idLabel='';var $newEntry=null;var $eid=null;var $modelID=null;
var $style=null;var $align=null;
var $data=null;
var $element=null;
var $formName=null;
var $wid=0;var $yid=0;var $hasTabs=false; 
var $content='';
var $crlf=''; 
var $extraClass='';
var $elementClassPosition='';
...
$this->_isFormExist();
if(isset($this->formName) && !empty($complexMap)) WPage::addJSScript("jCore.req['".$this->formName."']['" . $complexMap . "']='" . WTools::parseJSText(ltrim($requiredTypes,',')). " ; ';", 'validation', false);
}else{
$this->_isFormExist();
if(isset($this->formName) && !empty($complexMap)) WPage::addJSScript( "jCore.req['".$this->formName."']['" . $complexMap . "']=1;", 'validation', false);
}
}
}
...
WPage::addJSScript( "jCore.req['". $this->formName."']['".$map."']=1;", 'validation', false);
}
}
...
$this->adminE('The advance search module need to be setup before the "Link to Seach" feature can works!');
}
}
}
}
