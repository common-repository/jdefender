...
var $name='';var $filtersHTML=null;var $colNb=0; var $pKeyMap=null;var $multiplePK=false;
var $champsA=array();var $pageNavO=null;var $picklistDefaultA=null;var $manualDataB=false;
var $limitStart=0;
var $limitMax=0;
var $totalItems=0;var $searchWord='';
var $searchOnB=false;
private $_totalCount=0;
var $headNumberFooter=''; var $headListFooter=''; var $headSeachFooter=''; var $pagiHTML='';
var $pagiIncrement=null;
var $myParentA=array();var $parentB=false;var $totalElements=null;var $mapListA=array();
var $currentOrder=null;var $currentOrderDir=null;
var $orderingMap=null;var $orderingByGroup=null;private $_orderingExist=false;
var $queryInfoWhere=null; var $sid=0;
var $wid=0;
var $search=false;var $searchedColumnA=array();
var $advSearch=false;var $advSeachableA=array();
var $pagination=false;
var $dropdown=false;
var $mapParams='';
static public $indentID=0;
function create(){
...
if(empty($new)){
$new=WGlobals::get('search'.$this->yid, null, 'post','string');
...
$fitlersClass->addFilterToView($this, $this->_model, self::$removedConditionsA );
}if(!empty($this->champsA)){
for($index=0; $index < $sizeChamps; ++$index){
...
Output_Listings_class::$indentID=$listing->lid;
}
...
$message->codeE('A column cannot be advanced searchable if it does not have a sid '.$listing->sid.' or a valid map '.$listing->map );
}}
if(!array_key_exists($dataMap, $select ) && $selectMe){
if($listing->typeName =='image'){
$fileSID=WModel::get('files','sid');
$select=array('name','type','path','height','width','theight','twidth','thumbnail','storage','secure','modified');
...
if(!empty($myNewElements))$this->elements=$myNewElements;
...
$textlink2=(!empty($listing->textlink)?$listing->textlink : '');
WTools::getParams($listing );
if(!empty($textlink2)){
$listing->textlink=$textlink2;
}
if(!empty($listing->requirednode)){
$nodeExist=WExtension::exist($listing->requirednode );
if(empty($nodeExist)){
unset($this->elements[$lKid] );
continue;
}}
...
if(!empty($this->_model) && 40==$this->_model->getModelInfo('type')){
$newOrderA=array();
$orderinA=array();
$parentA=array();
$prtMP=$this->_model->getModelInfo('parentmap');
$prtMap=(!empty($prtMP)?$prtMP : 'parent'). '_'.$this->sid;
$pkMap=$this->_model->getPK(). '_'.$this->sid;
$ordMap='ordering_'.$this->sid;
$dpthMap='depth_'.$this->sid;
foreach($this->_data as $d){
if(!isset($d->$dpthMap)) continue;
if(!empty($d->$ordMap))$orderinA[$d->$pkMap]=$d->$ordMap;
if(!empty($d->$pkMap))$parentA[$d->$pkMap]=$d;
}
if(!empty($orderinA )){
$parentOrderA=array();
$maxDepth=10;
...
if(!isset($d->$dpthMap) || $d->$dpthMap !=$i ) continue;
$ord=$this->_getOrderTree($d, $count, $prtMap, $dpthMap, $parentOrderA, $maxDepth );
$parentOrderA[$d->$pkMap]=$ord;
$newOrderA[$ord]=$d;
}
}ksort($newOrderA );
$this->_data=$newOrderA;
}
}
...
$this->removeElementsProcess( self::$removedElementsA, true, 'output_listings_class');
...
if(!empty($this->mapParams))$listingI->paramsMapName=$this->mapParams;
...
private function _getOrderTree($d,&$count,$prtMap,$dpthMap,$parentOrderA,$maxDepth){
if(!isset($d->$prtMap) || !isset($d->$dpthMap )){
static $countMx=0;
$countMx++;
return $count*$countMx;
}
static $orderingA=array();
$count++;
if(!isset($parentOrderA[$d->$prtMap])){
$parentOrder=0;
}else{
$parentOrder=$parentOrderA[$d->$prtMap];
}
$ord=($count * ( pow( 10, $maxDepth - $d->$dpthMap )) + $parentOrder );
$ord=$this->_checkOrdering($ord, $orderingA );
$orderingA[$ord]=true;
return $ord;
}
private function _checkOrdering($ord,$orderingA){
if(!isset($orderingA[$ord])) return $ord;
$ord++;
return $this->_checkOrdering($ord, $orderingA );
}
...
$key='listingsA|'.$yid;
if(isset(WView::$safeModeViewElementsA[$key])){
return WView::$safeModeViewElementsA[$key];
}return false;
}
