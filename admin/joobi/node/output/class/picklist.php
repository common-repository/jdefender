...
var $defaults=null; var $params=null; 
var $disabled=false;
...
if(!is_array($dIds ) && ! is_numeric($dIds)){
if( is_string($dIds) && strpos($dIds, '#')===false){
$dIdsA=explode('_',$dIds );
$dIds='#'.$dIdsA[0].'#'.$dIds;
}}
}
...
if('data'==$property){
return $this->_didLists[0];
}if(!empty($this->_didLists[0]->$property )){
return $this->_didLists[0]->$property;
}
return null;
}
...
$this->_classes=((isset($this->params->classes))?$this->params->classes : '');
if(empty($this->_classes) && !empty($dropdown->classes))$this->_classes=$dropdown->classes;
...
$search=(!empty($dropdown->search )?$dropdown->search : 0 );
$this->_Hdrop=$HTMLDrop->create($this->_oneDrop, $dropdownName, $this->_htmlType, $this->_idName, $this->_captionName, $this->_initialValue, $dropdownTagId, $this->translate, $this->_arrayType, false, $this->disabled, $search );
break;
case '2' : $colnb=(!empty($dropdown->colnb))?$dropdown->colnb : 0;
$HTMLDrop=new WRadio();
if(!empty($this->_classes))$HTMLDrop->classes=$this->_classes;
...
break;
default :
$this->codeE('Pick-list type not defined : '.$dropdown->outype );
return false;
break;
}
...
if(!isset($oneEntry->value)) continue;
...
public function diaplyOneHTML($myValue=0){
$result=$this->displayOne($myValue );
if($result->status){
...
if( WGet::maintenanceMode()){
if(!empty($dropdown->valuesA)){
foreach($dropdown->valuesA as $OValue){
if(is_array($this->defaults) && in_array($OValue['value'], $this->defaults)){
$result[]=WText::t($OValue['name'] );
...
$baseLocation=JOOBI_DS_USER.'custom/';
}else{
...
WMessage::log($droplist  , 'picklist-file-missing_error');
$tmp=null;
return $tmp;
}
...
if(isset($didA[$id] )){
$x=$didA[$id];
if(!empty($tempdataA[$x])){
$this->_didLists[]=$tempdataA[$didA[$id]];
continue;
}}}else{
if(isset($tempdataA[$id])){
$this->_didLists[]=$tempdataA[$id];
continue;
}
}}
$tempdata=WViews::loadDataPicklist($id );
if(!empty($tempdata)){
$this->_didLists[]=$tempdata;
}else{
WMessage::log('processData PICKLIST 2: '.$id , 'picklist_not_found_debug');
WMessage::log($dIds , 'picklist_not_found_debug');
}
}
}else{
$getModel=true;
...
if( WGet::maintenanceMode()){
if( is_string($myInherit )){
$expA=explode('_',$myInherit );
$this->_didLists[$myKey]->inherit=WGet::loadData($expA[0].'.'.$myInherit , 'picklist');
if(!empty($this->_didLists[$myKey]->inherit->valuesA)){
$this->_didLists[$myKey]->valuesA=$this->_didLists[$myKey]->inherit->valuesA;
$myValsA=$this->_didLists[$myKey]->inherit->valuesA;
$myNewA=array();
foreach($myValsA as $val){
$obj=new stdClass;
foreach($val as $p=> $v){
if('name'==$p)$v=WText::t($v );
$obj->$p=$v;
}
$myNewA[]=$obj;
}
$idx='valuesA'.'|'.$this->_didLists[$myKey]->inherit->did;
WView::$safeModeViewElementsA[$idx]=$myNewA;
}
}
}else{
...
if(!isset($oneResult->rolid) || ! in_array($oneResult->rolid, $myRoleA )){
if(!WGet::readConfig('install')) unset($this->_didLists[$myKey] );
}
}
}
}
...
if($short){
$picklistM->select( array('did','rolid','core','namekey','sid','ref_sid','type','outype','external','allowothers','first_value','first_caption','params','map','parent','isparent','first_all','lib_ext','wid','reload','colorstyle','params'));
}
...
if( WGet::maintenanceMode()){
$nameky='';
foreach($this->_didLists as $didO){
if($didO->did==$values[0]){
$nameky=$didO->namekey;
}}$idx='valuesA'.'|'.$values[0];
if(isset(WView::$safeModeViewElementsA[$idx])){
$valuesA=WView::$safeModeViewElementsA[$idx];
foreach($valuesA as $nb=> $val){
$valuesA[$nb]->picklist=$nameky;
$valuesA[$nb]->did=$values[0];
}
$picklistsValuesA[$key]=WView::$safeModeViewElementsA[$idx];
}
}else{
$picklistValuesM=WModel::get('library.picklistvalues');
...
if(!isset($this->defaults) && ! isset($this->defaults[$pickvalue->did]))$this->defaults[$pickvalue->did]=$pickvalue->value;
...
if(!isset($this->defaults))$this->defaults[$sampleDID]=$this->_hasOtherInputBox;
$this->_hasOtherSpecificValue=true;
}}else{
foreach($picklistsValuesA[$key] as $pickvalue){
if(!isset($this->defaults) && empty($valueExitA[$pickvalue->did]) && isset($defaultsValueA[$pickvalue->did]))$this->defaults[$pickvalue->did]=$defaultsValueA[$pickvalue->did];
}
}
return true;
}
