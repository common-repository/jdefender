...
function create($listA,$tag_name,$tag_attribs=null,$propKey='value',$propText='text',$selected=null,$idtag=null,$translate=false,$arrayType=false,$onChange=false,$disabled=false,$search=0){
static $addJS=true;
static $count=1;
$useFancy=( count($listA) > 9?true : false);
$picklistsearch=WPref::load('PMAIN_NODE_PICKLISTSEARCH');
if('never'==$picklistsearch){
$useFancy=false;
}elseif('always'==$picklistsearch){
$useFancy=true;
}if(!empty($search )){
if( 9==$search)$useFancy=true;
else $useFancy=false;
}
...
if($viewType==51 ) return $this->createCollapsableTree($listA, $tag_name, $tag_attribs, $propKey, $propText, $selected, $idtag, $translate, $arrayType, $onChange );
}}
...
if(isset($listA[$key]->text))$listA[$key]->text=WGlobals::filter($value->text, 'safejs');
}
$count++;
...
$this->content .=$this->_options($listA, $propKey, $propText, $selected, $translate, $arrayType );
$this->content .='</select>';
$this->content .=WGet::$rLine;
return $this->content;
}
...
$value=WGlobals::filter($value, 'safejs');
...
if(!empty($this->classes))$data->classes=$this->classes;
...
if(!empty($tmpl) && 'component'==$tmpl){
$this->hidden('tmpl','component');
}}
