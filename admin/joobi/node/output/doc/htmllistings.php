...
var $subTotalName=array();
var $subTotalColmun=array();
var $subAverageColmun=array();
private $_graph3axeColumnUsed='';private $_alreadyChecked3DGraph=false;
var $paramsMapName='';
var $tableDetailsO=null;
...
|| ( WRoles::isNotAdmin('manager') && $this->htmlObj->pagination==1 ))){$html .=$this->tableDetailsO->transform->createBottom($this->htmlObj );
}
...
if(!empty($listing->hidden )) continue;
...
switch ($listingType){
case 'text':$header=$this->_ordering($header, $mapsid, $ordering, $listing->description );
break;
case 'input':
break;
case 'rownumber':
...
break;
case 'order':
if($listing->map=='ordering' && !empty($listing->sid)){
$this->htmlObj->colNb++;
...
$valueA=array('zsid'=> $listing->sid, 'lstg'=>true, 'zact'=>'all','total'=> $num );
...
if(empty($tempArray[$index])) return '';$this->allDataRowsA=$tempArray[$index];}}
...
$originTR_c=(!empty($t->tr_c)?$t->tr_c : '');
...
$asCore=false;
if(!empty($this->htmlObj->mapListA['core'] )){
$coreMap=$this->htmlObj->mapListA['core'];
if(empty($row->$coreMap)){
if(!empty($t->tr_c))$t->tr_c .=' ';
$t->tr_c .='danger';
$asCore=true;
}}if(!$asCore && !empty($this->htmlObj->mapListA['publish'] )){
$coreMap=$this->htmlObj->mapListA['publish'];
if(empty($row->$coreMap)){
if(!empty($t->tr_c))$t->tr_c .=' ';
$t->tr_c .='warning';
}}
...
$this->cellValue=0;if(isset($this->htmlObj->head) && $this->htmlObj->head ){
...
$titleHeader=(!empty($description))?$description : WText::t('1206732366OVLX');$h .='href="#" onclick="'.$joobiRun.'" title="'.WGlobals::filter($titleHeader, 'string').'">';
$h .=$text.' ';
