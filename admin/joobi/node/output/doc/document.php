...
var $htmlObj=null; 
var $cellValue=''; var $complexMap=''; var $rowNumber=0;var $rowOffset=0;var $numberRows=0;protected $displayCategoryRoot=false; var $allDataRowsA=array();protected $onlyOnceOrdering=true;
var $paramsMapName='';
...
$key='picklistA|'.$this->htmlObj->yid;
if(isset(WView::$safeModeViewElementsA[$key])){
$pickA=WView::$safeModeViewElementsA[$key];
if(!empty($pickA)){
$storePicklistA[$this->htmlObj->yid]=array();
foreach($pickA as $onePcik){
if( is_object($onePcik->did)){
$storePicklistA[$this->htmlObj->yid][]=WView::picklist($onePcik->did->_didLists[0]->did, '', null, 'did');
}else{
$storePicklistA[$this->htmlObj->yid][]=WView::picklist($onePcik->did, '', null, 'did');
}}}
}
}else{
...
$eid=WGlobals::getEID();
if(!empty($eid))$resetLink .='&eid='.$eid;
...
if(!empty($task))$formHTML->hidden('task',$task );
if(!empty($eid))$formHTML->hidden('eid',$eid );
