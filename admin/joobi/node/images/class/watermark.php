...
if(empty($fontFile )){
$validFont=JOOBI_DS_MEDIA.'fonts/monofont.ttf';
}else{
$waterImgPath=WPref::load('PIMAGES_NODE_WATERMARKTEXTFONTFILE');
$waterImgPath=JOOBI_DS_ROOT . trim( str_replace( array('/','\\'), '/',$waterImgPath ), '/');
$fileC=WGet::file();
...
if(!isset($validImg)){
$waterImgPath=WPref::load('PIMAGES_NODE_WATERMARKIMAGEFILE');$waterImgPath=JOOBI_DS_ROOT . trim( str_replace( array('/','\\'), '/',$waterImgPath ), '/');
$fileC=WGet::file();
if(!$fileC->exist($waterImgPath )){
$validImg=true;
$message=WMessage::get();
