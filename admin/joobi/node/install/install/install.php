...
self::logMessage( 'Now processsing extension: ' . $package->temp_folder , 'install', false, 1, true, true);
			...
			$themeFolder = $package->temp_folder . '/theme';
...
self::logMessage( 'importing languages','install', false, 1, true, true);
			...
			$translationHandler->setForceInsert( true );
				...
				if ( empty( $atInstallDistribsite ) ) {
					$atInstallDistribsite = WPref::load( 'PINSTALL_NODE_MAINDISTRIB' );
				}			} else {
...
self::logMessage( 'populating languages', 'install' );
...
self::logMessage( 'reset the joobi cache','install', false, 1, true, true);
	...
	public static function third_install($installerO) {
		...
		$user = JOOBI_DS_ROOT.JOOBI_FOLDER . '/user/';
		$file_handler->write( $user.'safe/.htaccess', 'Deny from all', 'force' );
		$file_handler->write( $user.'logs/.htaccess', 'Deny from all', 'force' );
...
self::logMessage( 'load real tables','install' );
		...
		$curentCount = 0;
			...
			$doneTablesA[] = $k;
			$real_folder = rtrim( $folder,DS ) . '/database/real';
...
self::logMessage( 'All tables created in the database.' ,'install', false, 1, false, false );
	...
	private static function extractCorePackages(&$install) {
		...
		foreach( $joobiCorePackagesA as $i => $filename ) {
			...
			$folder = JOOBI_DS_ROOT. JOOBI_FOLDER . '/' . ( $filename[1] != '' ? str_replace( '.', '/', trim( $filename[1] ) ) . '/' : '' ) . $name;
			$joobiCoreA[$i] = $folder;
			if ( !file_exists( $path ) ) {
...
echo 'error in the processing of '.$namekey;
	...
	public static function mkdir($folder) {
		...
		@touch(rtrim($folder,DS) . '/index.html' );
		return true;
	}
...
self::logMessage( 'XML info retrieved','install', false, 1, true, true );
	...
	public static function moveRootFiles($destination='') {
		...
		$joobi_root_files = $install_folder . '/files/';
				$joobi_joobi = JOOBI_DS_JOOBI;
		if ( !empty($destination) ) {
			$joobi_joobi = dirname( dirname($destination) ) . '/';
		}
self::logMessage( 'moving entry files from '. $joobi_root_files. ' to ' . $joobi_joobi, 'install', false, 1, true, true);
		...
		if ( $folder_handler->exist( $joobi_root_files ) ) {
			self::logMessage( 'entry files folder exists', 'install' );
...
self::logMessage( 'ERROR ERROR ERROR ERROR the file '. $joobi_root_files . $file . ' could not be moved to ' . $joobi_joobi . $file, 'install' );
					return false;
				}			}
	...
	private static function _loadCommon() {
		if ( !class_exists('Install_Common_class') ) require_once( dirname(dirname(__FILE__) ) . '/class/common.php' );
	}
	public static function generateFirstDefines() {
				if ( !defined('JOOBI_DS_NODE') ) define( 'JOOBI_DS_NODE', JOOBI_DS_ROOT.JOOBI_FOLDER . '/node/' );
	...
	private static function loadFromFileCache($action,$location,$data=null) {
				if ( !defined('JOOBI_DS_ROOT') ) define( 'JOOBI_DS_ROOT', dirname(JOOBI_DS_JOOBI) . '/' );
		$filePath = JOOBI_DS_ROOT . 'tmp/' . strtolower($location) . '.jti';
		if ( $action == 'exist' ) {
			...
			$exists = Install_Node_install::loadFromFileCache( 'exist', $location );
...
self::logMessage( 'In improveInstallProcess 2', 'install', false, 1, true, true );
			$file = dirname( dirname(__FILE__) ) . '/class/process.php';
			$new_file = JOOBI_DS_JOOBI . 'node/install/class/process.php';
			$filehandler = WGet::file();
			$filehandler->copy( $file, $new_file, 'force' );
			return true;
	}
	private function _removeOldXmlFiles(){
		$xml_file = JOOBI_DS_JOOBI . 'node/files/xml/extension_dependency.xml';
		$xml_file2 = JOOBI_DS_JOOBI . 'node/captcha/addon/image/xml/extension_dependency.xml';
		$filehandler = WGet::file();
		if ($filehandler->exist($xml_file)&&$filehandler->exist($xml_file2)){
			$old_install_file = JOOBI_DS_JOOBI . 'node/install/class/package.php';
			$new_install_file = dirname(dirname(__FILE__)) . '/class/package.php';
...
self::logMessage( 'In updateInstallProcessCompatibility2 1','install', false, 1, true, true);
		...
		$new_folder = JOOBI_DS_JOOBI . 'node/install';
		$systemFolderC = WGet::folder();
