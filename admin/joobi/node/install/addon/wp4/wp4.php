...
class Install_Wp4_addon {
	...
	public function setExtensionInfo($namekey) {
		...
		$explodeNamekeyA = explode( '.', $namekey );
								$path = 'node/' . $explodeNamekeyA[0] . '/' . $explodeNamekeyA[1];
		$this->path = JOOBI_DS_ROOT . JOOBI_FOLDER . '/' . $path;
		$name = $extension->name;
...
';
		$path = WP_PLUGIN_DIR . '/' . $pluginSlup . '/';
		$pathFull = $path . $pluginSlup . '.php';
		$fileS = WGet::file();
		$fileS->write( $pathFull, $phpcode, 'force' );
				$fileS->copy( dirname(__FILE__) . '/tar.php', $path . 'tar.php', 'force' );
				$status = $fileS->copy( dirname(__FILE__) . '/install.php', $path . 'install.php', 'force' );
		return true;
	}
	function getMainTag(){
		return 'extension';
	...
	public function setUninstallFile($namekey) {
		...
		$uninstallfile = JOOBI_DS_NODE.'api/addon/' . $this->apiVersion . '/uninstall.php';
		$explodeNamekeyA = explode( '.', $namekey );
		...
		if ( $status ) {
			$uninstallfile = JOOBI_DS_NODE.'api/addon/' . $this->apiVersion . '/install.php';
			$explodeNamekeyA = explode( '.', $namekey );
			$namekeyFCT = '_' . strtoupper( $explodeNamekeyA[0] );
