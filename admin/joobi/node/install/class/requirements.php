...
class Install_Requirements_class {
	...
	public function checkRequirements() {
		...
		if ( 'fail' == $this->_finalStatus ) {
			$failListA = array();
			foreach( self::$_requirementsA as $paneA ) {
				if ( empty($paneA->paneA) ) continue;
				foreach( $paneA->paneA as $req ) {
					if ( 'fail' == $req->status ) {
						$failListA[] = $req->name . ': ' . $req->description;
					}
				}
			}
			return $failListA;
	...
	private function _serverDisplay() {
		...
		if ( empty($sqlVersion) ) {
			...
			if ( version_compare( $sqlVersion, '5.5.3', '>=' ) ) {	
				$status = 'success';
			} else {
				$status = 'fail';
			}
		...
		$elementA[] = $this->_addElement( 'p[phar]', 'Phar ' . WText::t('1432423407JSWL'), $text, $status );
		...
		$elementA[] = $this->_addElement( 'p[domdocument]', 'DOMDocument ' . WText::t('1433250455EAMT'), $text, $status );
		return $elementA;
	}
	private function _phpDisplay() {
		...
		$desc = '';
		if ( ini_get( 'opcache.enable' ) && !ini_get( 'validate.timestamps' ) ) {
			$status = 'warning';
			$text = $this->_No;
			$desc = WText::t('1476773122HDZD');
		} else {
			$status = 'success';
			$text = $this->_Yes;
		}
		$elementA[] = $this->_addElement( 'p[opcache]', 'OPcache ' . WText::t('1476825200QSNB'), $text, $status, $desc );
		return $elementA;
	}
	...
	private function _folderDisplay() {
		...
		foreach( $foldersA as $folder ) {
			$path = JOOBI_DS_ROOT . str_replace( '/', '/', $folder );
			$count++;
			if ( ! file_exists( $path ) ) {
				$folderS = WGet::folder();
				$folderS->create( $path );
	...
	private function _getMysqlVers() {
		if( function_exists( 'mysqli_connect' ) && $connect = @mysqli_connect( JOOBI_DB_HOSTNAME, JOOBI_DB_USER, JOOBI_DB_PASS) ) {
			$versA = array( mysqli_get_server_info( $connect ), mysqli_get_client_info() );
		} else {
			if( function_exists( 'mysql_connect' ) && $connect = @mysql_connect( JOOBI_DB_HOSTNAME, JOOBI_DB_USER, JOOBI_DB_PASS) ) {
