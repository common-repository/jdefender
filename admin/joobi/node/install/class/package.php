...
class Install_Package_class {
	var $errors = array();
	var $installUpdate = 1; 		var $newInstall = true;
	var $level = 0; 	var $auto = 3;
	var $main = true;
	...
	public static function installFilesFolder() {
		...
		if ( $fileS->exist( JOOBI_DS_NODE . 'install/class/processnew.php' )
...
self::logMessage( 'The extension_node.xml file is missing in the folder '.$this->temp_folder,'install' );
				return false;
			}
			if ( !array_key_exists('extension_version',$this->tables) || count($this->tables['extension_version'])<1){
self::logMessage( 'The extension_version.xml file is missing in the folder '.$this->temp_folder, 'install' );
				return false;
			}
			return true;
	}
	...
	public function triggerPreInstall(){
		if ( ! $this->initCustomInstance() ) {
			return false;
		}
self::logMessage( 'Trigger preinstall in triggerPreInstall() ','install' );
				if ( !$this->common->trigger( $this->install_ext, $this, 'preinstall') ) {
			return false;
		}self::logMessage( 'Pre-install triggered in triggerPreInstall() ','install' );
		return true;
	}
	public function initCustomInstance() {
self::logMessage( 'Initializing custom functions', 'install' );
self::logMessage( 'temp folder: ' . $this->temp_folder, 'install' );
...
self::logMessage( 'Custom functions initialized', 'install' );
		return true;
	}
	function getDestination($full=true) {
		if ( empty($this->mainext->folder) ) return '';
		$path = ( $this->mainext->destination != '' ? str_replace('|', '/', $this->mainext->destination) . '/' : '' ) . $this->mainext->folder;
		if ($full){
			return JOOBI_DS_ROOT.JOOBI_FOLDER . '/' . $path;
		}
		return $path;
	}
	function process() {
...
self::logMessage( 'starting triggers for methods '.implode(',',$methods),'install' );
		if ( !$this->common->trigger( $this->install_ext, $this, $methods ) ) {
			return false;
		}
self::logMessage( 'triggers finished','install' );
		return true;
	}
...
self::logMessage( 'Triggering cms specific function for the ' . $type . ' , namekey: ' . $this->mainext->namekey, 'install' );
				if ( ! $addon->$type() ) {
...
self::logMessage( 'SUCCESS: Triggering cms specific function', 'install' );
	...
	public function loadRealSQL() {
		$realFolder = rtrim( $this->temp_folder, '/' ) . '/database/real';
		$systemFolderC = WGet::folder();
...
self::logMessage( 'Custom functions loaded in extract()', 'install' );
						$dest = ( $this->mainext->destination != '' ? str_replace( '|', DS , $this->mainext->destination) . '/' : '' ) . $this->mainext->folder;
			$this->destination = Install_Package_class::installFilesFolder() . '/' . $dest;
...
self::logMessage( 'Files copied from ' . $temp_folder . ' to ' . $this->destination, 'install' );
		$old_xml_folder = JOOBI_DS_JOOBI . $dest . '/xml';
...
self::logMessage( 'Delete tmp folder ' . $temp_folder, 'install' );
	...
	function xml($folder) {
		$folder =	rtrim( $folder, '/' );
				$cache_folder = JOOBI_DS_TEMP . 'cache/xml/';
		...
		$systemFolderC = WGet::folder();
		$metadataLocation = $folder . '/metadata.xml';
self::logMessage( 'XML metadata file: ' . $metadataLocation, 'install' );
				$joobiWidgets = false;
		$joobiXml = $folder . '/xml/extension_node.xml';
		if ( $filehandler->exist( $joobiXml ) ) $joobiWidgets = true;
			...
			$xml = $parser->loadFile( $metadataLocation );
						if ( $xml[0]['nodename']=='data' && isset($xml[0]['attributes']['version']) ) {
								$data['data'] = $xml;
self::logMessage( 'Using metadata file', 'install' );
...
self::logMessage( 'ERROR: The extension xml data files are missing in the folder ' . $folder, 'install' );
			...
			if ( !is_array($xml) ) {
				self::logMessage( 'ERROR: Could not read the xml file '.$xmlfolder.$file,'install' );
				$this->_tryCMSPackageInstall( $folder );
				return false;
			}
			$table = array();
...
self::logMessage( 'call updatePref 1', 'install' );
	...
	function checkLicense() {
		...
		if ( $ext->level > 0 ) {
			...
			if ( !$licenceValid ) self::logMessage( 'License not valid because: ' . $reason, 'install' );
			return true;
		}
...
self::logMessage( 'Checking already installed version: ' . $this->mainext->namekey . ' - ' . $this->current_version, 'install' );
...
self::logMessage( ' Checking checkDependencies() start ', 'install' );
self::logMessage( ' with old wid: '.$this->old_wid,'install' );
...
self::logMessage( 'Could not find the extension data for one of the extension child of the extension '.$this->mainext->namekey, 'install' );
						continue;
					}
...
self::logMessage($myMess, 'install' );
						return false;
					}
				if ( ( $this->use_core_array && $joobiCoreA[$key]=='finished') || $this->list[$key]['install'] == 1 ) continue;
												$this->child_key = $key;
...
self::logMessage( ' Checking checkDependencies() finished ', 'install' );
		return true;
	}
	private function _statistics() {
		...
		$distributionSite = WPref::load( 'PINSTALL_NODE_MAINDISTRIB' );
		$netcom = WNetcom::get();
