...
class Install_Themefile_class extends WClasses {
 	public function addThemeFile($type,$filesA) {
		...
		foreach( $this->_allThemeA as $oneTheme ) {
			...
			$baseOrignal = $basePath . $location . '/' . $original . '/';
			$baseDestination = $basePath . $location . '/' . $oneTheme->folder . '/';
			foreach( $filesA as $oneFile ) {
				if ( substr( $oneFile, -3 ) == '.js' ) {
					$oneFile = str_replace( '.js', '.min.js', $oneFile );
				}
				if ( substr( $oneFile, -4 ) == '.css' ) {
					$oneFile = str_replace( '.js', '.min.js', $oneFile );
				}
