...
class Install_Common_class {
	...
	var $namekey = '';	
	function process() {
self::logMessage( '--Extension name -- ' . $this->_getExtensionIdentifier(), 'install' );
...
self::logMessage( '++An error occurred in the type method '.$function.' for the extension '.$extension,'install' );
				}
			}
			return true;
		}
	...
	public static function writeFrameworkDefaultConfigFile() {
		$folder = JOOBI_DS_ROOT . JOOBI_FOLDER . '/';
...
var $multiDB = false;
var $secret = \'' . WTools::randomString( 29, true ) . '\';
}//endclass';
...
self::logMessage( 'Trigger custom function ' . $v, 'install' );
				if ( Install_Common_class::trigger( $install_ext, $install, $v ) ===false ) {
self::logMessage( 'FAILED: Trigger custom function ' . $v, 'install' );
					return false;
				}
			}
...
self::logMessage( 'triggering custom function ' . $method, 'install' );
...
self::logMessage( 'custom install object could not be loaded', 'install' );
		}
		return true;
	}
	...
	public function initCustom(&$install_ext) {
		require_once( JOOBI_DS_JOOBI . '/node/install' . '/class/install.php' );
		$inst_file = $this->real_folder . '/install/install.php';
		$proc = WGet::file();
...
WMessage::log( 'process Import data  _processDataSQLFiles 9', 'install' );
		...
		$folder = rtrim( $this->real_folder, '/' ) . '/database';
		$file = $folder . '/data/' . $this->namekey . '_data_mysql.sql';
