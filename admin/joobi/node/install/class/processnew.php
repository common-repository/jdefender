...
class Install_Processnew_class {
	...
	private $_isjApps = false;
	private $_returnedMessageA = array();
	private $_packageToInstallA = array();
	private $_libraryNodeA = array( 'api.node', 'apps.node', 'category.node', 'editor.node', 'email.node', 'files.node', 'images.node', 'install.node',
			'japps.application', 'joomla.node', 'library.node', 'main.node', 'netcom.node', 'output.node', 'role.node', 'theme.node', 'translation.node', 'users.node' );
	private $_installDataStatus = false;
	...
	private function _getMessage() {
 	...
 	public function getListOfApps() {
Install_Processnew_class::logMessage( 'List of Apps to install language.' );
 		$extensionM = WTable::get( 'extension_node', 'main_library', 'wid' );
 		$extensionM->whereE( 'type', 1 );
 		$extensionM->whereE( 'publish', 1 );
 		$this->_packageToInstallA = $extensionM->load( 'ol', array( 'wid', 'namekey', 'version' ) );
Install_Processnew_class::logMessage( $this->_packageToInstallA );
 		if ( empty($this->_packageToInstallA) ) {
 			$this->_setMessage( 'html', WText::t('1470263724ATVE'), 'failed' );
 		} else {
 			$this->_setMessage( 'html', WText::t('1470263724ATVF'), 'success' );
 		} 		
 	...
 	public function getNumberPackage() {
		$packA = Install_Node_install::accessInstallData( 'get', 'installExtension' );
Install_Processnew_class::logMessage( 'List of package retrieved:' );
Install_Processnew_class::logMessage( count( $packA ) );
Install_Processnew_class::logMessage( count( $this->_packageToInstallA ) );
 		return count( $packA );
 	}
 	...
 	public function installLanguages($nb) {
		static $createTbleB = true;
 		 		$importLangsA = Install_Node_install::accessInstallData( 'get', 'importLangs' );
				$translationPopulateM = WModel::get( 'translation.populate' );
		$translationPopulateM->whereE( 'eid', 0 );
		$translationPopulateM->operator( 'OR' );
		$translationPopulateM->whereE( 'imac', '' );
		$translationPopulateM->operator( 'OR' );
		$translationPopulateM->whereE( 'dbcid', 0 );
		$translationPopulateM->delete();
		if ( $createTbleB && !empty($importLangsA) ) {
			$translationChecktableC = WClass::get( 'translation.checktable' );
			foreach( $importLangsA as $lang ) {
				if ( empty($lang->code) ) continue;
				if ( 'en' == $lang->code ) continue;
								$transExist = $translationChecktableC->transTableExist( $lang->code );
								if ( ! $transExist ) {
					$translationChecktableC->createTransTable( $lang->code );
				}
			}				
			$createTbleB = false;
		}
 	...
 	public function populateLanguages($nb) {
 		$this->finish_translation( $nb );
 		return $this->_getMessage();
 	} 	
 	...
 	public function installData($nb,$type,&$status) {
 		$this->_installDataStatus = false;
 		$this->_installData( $nb, $type );
 		$status = $this->_installDataStatus;
 		return $this->_getMessage();
 	} 	
...
Install_Processnew_class::logMessage( 'Moving folder: ' . JOOBI_DS_USER . 'installfiles/' . $folder, 'install' );
			if ( $systemFolderC->exist( JOOBI_DS_USER . 'installfiles/' . $folder ) ) {
				$systemFolderC->copy( JOOBI_DS_USER . 'installfiles/' . $folder, JOOBI_DS_JOOBI . $folder, 'add_over' );
			}
			if ( $systemFolderC->exist( JOOBI_DS_USER . 'installfiles/' . $folder ) ) {
				$systemFolderC->delete( JOOBI_DS_USER . 'installfiles/' . $folder );
			}
		}
...
self::logMessage( '-- Finish: addUpdateDependencies --', 'install' );
 ...
 self::logMessage( 'All CMS links created','install' );
 	...
 	function finish_translation($nb) {
		...
		$importLangs = Install_Node_install::accessInstallData( 'get', 'importLangs' );
			...
			if ( !empty($myLang) ) {
				...
				$translationProcessC->setStep( 1 );
				$translationProcessC->importLangs = $toProcessA;
				$status = $translationProcessC->populateTranslation();
				$LANGUAGE = $myLang->name;
				if ( $status ) {
					$this->_setMessage( 'append', '<br>' . str_replace(array('$LANGUAGE'), array($LANGUAGE),WText::t('1473681558DKGN')) );
				} else {
					$this->_setMessage( 'append', '<br>' . str_replace(array('$LANGUAGE'), array($LANGUAGE),WText::t('1473776358BRLI')) );
				}				
			} else {
				$installWidget = WGlobals::getSession( 'webapps', 'widgetinstall', null );
				if ( !empty($installWidget) && $installWidget !== true ) {
	...
	public function finishDownloadPackages() {									$this->updatePref( 2 );
 	...
 	public function removeXMLCacheFolder() {
 		$systemFolderC = WGet::folder();
		$systemFolderC->delete( JOOBI_DS_TEMP.'cache/xml' );
...
self::logMessage( $package . ' already installed ! Going to the next one' , 'install' );
			...
			if ( isset($install->switch_child) && $install->switch_child ) {
				...
				self::logMessage( 'The package '.$package.' requires first the install of the package '.$child['filename'],'install' );
...
self::logMessage( 'Calling activateExtensionVersionAndClean 2', 'install' );
		...
		$this->list[$k][$k2]['install'] = 1;
self::logMessage( 'The package "' . $package . '" was installed.', 'install' );
				if ( isset($_SESSION['joobi']) && isset($_SESSION['joobi']['install_status']) ) $this->updatePref( $_SESSION['joobi']['install_status'] );
 	...
 	function copy(&$return,&$v,&$v2,&$packages,$k,$k2) {
		$pieces = explode( DS, $v2['filename'] );
		$package = $pieces[ count($pieces)-1 ];
		$real_folder = rtrim( $v2['folder'], '/' );
		$dest = rtrim( $v2['destination'], '/' );
		$tmp_folder = JOOBI_DS_USER . 'installfiles/' . $dest;
...
self::logMessage( 'The CMS links for the package '.$package.' were created.', 'install' );
 	...
 	public function custominstall(&$return,&$v,&$v2,&$packages,$k,$k2) {
		$pieces = explode(DS,$v2['filename']);
		$package = $pieces[count($pieces)-1];
self::logMessage( 'starting custom install for '.$package,'install' );
		if ( empty($v['level']) ) $v['level'] = 0;
		$install = WClass::get( 'install.package' );
		$install->level = (int)$v['level'];
		$install->packages =& $packages;
		...
		$this->list[$k][$k2]['custominstall'] = 1;
self::logMessage( 'The custom install for the package '. $package. ' was run.', 'install' );
				if ( isset($_SESSION['joobi']) && isset($_SESSION['joobi']['install_status']) ) $this->updatePref( $_SESSION['joobi']['install_status'] );
		$EXTENSION = $install->mainext->name;
 	...
 	function customtype(&$return,&$v,&$v2,&$packages,$k,$k2) {
		...
		$this->list[$k][$k2]['customtype'] = 1;
self::logMessage( 'The type custom install for the package '.$package.' was run.','install' );
				if ( isset($_SESSION['joobi']) && isset($_SESSION['joobi']['install_status']) ) $this->updatePref( $_SESSION['joobi']['install_status'] );
		$EXTENSION = $install->mainext->name;
 	...
 	function checklicense(&$return,&$v,&$v2,&$packages,$k,$k2) {
		...
		$install->loadXml();
 		if ( isset($v2['oldVersion']) ) {
			...
			$install->newInstall = false; 		}
		$this->list[$k][$k2]['checklicense'] = 1;
self::logMessage( 'The license of the package '.$package.' was validated.','install' );
				if ( isset($_SESSION['joobi']) && isset($_SESSION['joobi']['install_status']) ) $this->updatePref( $_SESSION['joobi']['install_status'] );
		$EXTENSION = $install->mainext->name;
...
self::logMessage( $message, 'install' );
	...
	function clean($removeFolder=true) {
		...
		Install_Node_install::accessInstallData( 'delete', 'importlangs' );
				$folderC = WGet::folder();
		$folderC->delete( JOOBI_DS_ROOT . 'tmp/cache' );			$folderC->create( JOOBI_DS_ROOT . 'tmp/cache' );
				$libProgreC = WClass::get( 'library.progress' );
		$progressO = $libProgreC->get( 'apps' );
		$libProgreC->deleteProgress();
				$cC = WCache::get();
		$cC->resetCache();
		return true;
	}
	function checkServerAvail($server='',$force=false) {
 	...
 	function extract(&$return,&$v,&$v2,&$packages,$k,$k2) {
		...
		self::logMessage( 'Extracting the package ' . $package, 'install' );
...
self::logMessage( 'The folder  ' . $this->list[$k][$k2]['folder'] ,'install' );
self::logMessage( 'The destination  ' . $this->list[$k][$k2]['destination'] ,'install' );
		if ( isset($_SESSION['joobi']) && isset($_SESSION['joobi']['install_status']) ) $this->updatePref( $_SESSION['joobi']['install_status'] );
				$parts=explode('_',$package);
		if ($parts[0]=='lib'){
			$parts[1]='lib';
	...
	public static function logMessage($message,$location='install',$not1=null,$not2=null,$not3=null,$not4=null) {
		...
		$installDebug = WPref::load( 'PINSTALL_NODE_INSTALLDEBUG' );
		if ( $installDebug ) {
...
self::logMessage( ' $folder= '.print_r($folder, true),  'error_' . __FUNCTION__ );
 	...
 	private function _overwriteEntryPoint() {
 		if ( JOOBI_FRAMEWORK_TYPE == 'joomla' ) {
			$cms_addon = JOOBI_DS_USER . 'installfiles/node/install/addon/' . JOOBI_FRAMEWORK . '/' . JOOBI_FRAMEWORK . '.php';
			$entry_file = JOOBI_DS_ROOT . 'administrator/components/com_' . JOOBI_MAIN_APP . '/' . JOOBI_MAIN_APP . '.php';
...
self::logMessage( 'skipping entry point overwrite, no cms addon file at ' . $cms_addon, 'install', false, 0, false  );
	...
	private function _getListOfPackages() {
		...
		self::logMessage( 'Send list request to '. $this->_getDistribSite() ,'install' );
		$netcom = WNetcom::get();
		...
		if ( 'single' == $what2Update ) {				$extensionNamekeySent = $app2InstallA[0]->namekey;
			...
			if ( empty($token) ) {
				$appInfosA = WTable::get( 'extension_userinfos', 'main_userdata', 'wid' );
				$appInfosA->makeLJ( 'extension_node', 'main_userdata', 'wid', 'wid' );
				$appInfosA->whereE( 'namekey', $extensionNamekeySent, 1 );
				$token = $appInfosA->load( 'lr', 'token' );
			}			
			...
			} elseif ( $what2Update == 'multipleinstall' ) {
				$sentData->type = 'multiple';									$appsM = WModel::get( 'install.apps' );
				$appsM->whereE( 'publish', 0 );
				$appsM->whereE( 'type', 1 );
				$app2InstallA = $appsM->load( 'ol', 'namekey' );				
			} else {
...
self::logMessage( 'There is no valid token to do this update!' ,'install' );
				...
				$newData = $this->_formatSendData( $alreadyInstalledA );
...
self::logMessage( 'Successfully receive the packages' ,'install' );
				...
				if ( is_array($unseraibleMe) ) {
					...
					$sortedExtA = array();
										...
										$skipPack = WPref::load( 'PINSTALL_NODE_SKIPPACK' );
					$mainExist = WExtension::exist( 'main.node' );
					if ( ! $mainExist ) $skipPack = 0;
					if ( $skipPack ) {
						$existingPackA = array();
						foreach( $alreadyInstalledA as $packO ) {
							$existingPackA[$packO->namekey] = $packO->version;
						}					}					
					foreach( $extensionReceviedA as $onePack ) {
						if ( $skipPack ) {
							if ( isset($existingPackA[$onePack->e]) && $onePack->n == $existingPackA[$onePack->e] ) {
																$expA = explode( '.', $onePack->e );
								$typeApp = array_pop( $expA );
								if ( 'application' != $typeApp ) continue;
								if ( ! $this->_isjApps && 'japps.application' == $onePack->e ) continue;
							}							
						}						
						$obj = new stdClass;
...
self::logMessage( 'package To InstallA', 'install' );
self::logMessage( $this->_packageToInstallA, 'install' );
					if ( !empty($this->_packageToInstallA) ) {
						$status = Install_Node_install::accessInstallData( 'set', 'installExtension', $this->_packageToInstallA );
						if ( ! $status ) {
					...
					self::logMessage( 'The distribution site could not provide the list of extensions for repository: ' . $receivedData->reposid, 'install' );
...
self::logMessage( 'Could not retrieve the list of packages: ' . $receivedData->reposid, 'install' );
self::logMessage( $receivedData, 'install' );
...
self::logMessage( 'Check selected applications' );
			...
			$this->_isjApps = false;
				...
				$wid = WForm::getPrev( 'wid' );
					...
					$appsM->whereE( 'namekey', JOOBI_MAIN_APP . '.application' );
			...
			if ( ! $this->_isjApps ) {
				...
				$appsM->rememberQuery( true );
				$appsM->whereE( 'namekey', JOOBI_MAIN_APP . '.application' );
				$jAppsID = $appsM->load( 'lr', 'wid' );
				if ( $wid == $jAppsID || @$to_install[0] == $jAppsID ) {
					$this->_isjApps = true;
				}					
			}				
			if ( !isset($to_install) ) {
...
self::logMessage( 'Get the packages already installed' );
	...
	private function _alreadyInstalled() {
		...
		$sql->whereIn( 'type' , array( 1, 150, 175, 101, 102, 103 ) );
...
Install_Processnew_class::logMessage( 'Extracting package ' . $PACKAGE_NAME, 'install' );		
				$filename = JOOBI_DS_USER . 'downloads/packages/' . str_replace( '.', '_', $extensionInfoO->namekey ) . '_' . $extensionInfoO->version . '.tar.gz';
		$installPackageC = WClass::get( 'install.package' );
...
Install_Processnew_class::logMessage( 'Extracting package result: ' . "The package $PACKAGE_NAME could not be extracted.", 'install' );			
	...
	private function _downloadLanguages($nb) {
		...
		$extensionNb = $nb - 1;
		if ( !isset($this->_packageToInstallA[$extensionNb]) ) {
			$this->_setMessage( 'html', 'Missing package number : ' . $extensionNb, 'failed' );
			return false;
		}		
		$extensionInfoO = $this->_packageToInstallA[$extensionNb];
		if ( empty($extensionInfoO) ) {
		...
		if ( $status ) {
			$this->_setMessage( 'append', '<br>' . str_replace(array('$PACKAGE_NAME'), array($PACKAGE_NAME),WText::t('1470794358HHWS')) );
		} else {
...
self::logMessage( 'starting CMS menu creation for ' . $PACKAGE_NAME, 'install' );
			...
			$install->loadXml();
self::logMessage( 'Publish extension : ' . $extensionInfoO->namekey , 'install' );			
			$extensionT = WTable::get( 'extension_node' );
			$extensionT->whereE( 'namekey', $extensionInfoO->namekey );
			$extensionT->setVal( 'publish', 1 );
			$extensionT->setVal( 'version', 'lversion', 0, 0 );
			$extensionT->update();
...
self::logMessage( 'Package type : ' . $explodeKeyA[1], 'install' );
		...
		$methods = $install->customInstallMethods();
				WText::load( $extensionInfoO->namekey );
		$status = $install->triggerMethods( $methods );
self::logMessage( 'The custom install for the package '. $PACKAGE_NAME . ' was run.', 'install' );
		if ( $status ) {
	...
	private function _installData($nb,$type) {
				$this->getPref();
		$extensionNb = $nb - 1;
		$extensionInfoO = $this->_packageToInstallA[$extensionNb];
		if ( empty($extensionInfoO) ) {
			$this->_setMessage( 'html', 'The package information was not found when inserting data...', 'failed' );
			return false;
		}		
				$installDataC = WClass::get( 'install.data' );
		$status = $installDataC->processData( $extensionInfoO->namekey, $type );
		$PACKAGE_NAME = $extensionInfoO->namekey;
		$TYPE = $type;
		if ( $status ) {
			self::logMessage( 'The data was inserted for extension ' . $extensionInfoO->namekey );
			$this->_setMessage( 'append', '<br>' . str_replace(array('$TYPE','$PACKAGE_NAME'), array($TYPE,$PACKAGE_NAME),WText::t('1469203640GQNL')) );
			$this->_installDataStatus = true;
		} else {
			$this->_setMessage( 'append', '<br>' . str_replace(array('$TYPE','$PACKAGE_NAME'), array($TYPE,$PACKAGE_NAME),WText::t('1475033761PYFK')) );
			$this->_installDataStatus = false;
								}		
		return true;
	}	
...
self::logMessage( 'Create tables for the package ' . $extensionInfoO->namekey, 'install' );
		...
		$file = $base . $explodeKeyA[0] . '/database/data/' . $extensionInfoO->namekey . '_data_mysql.sql';
		$fileS = WGet::file();
		if ( ! $fileS->exist( $file ) ) {
...
self::logMessage( 'Package ' . $PACKAGE_NAME . ' downloaded successfully', 'install' );
...
self::logMessage( 'Package download problem:', 'install' );
self::logMessage( $receivedData, 'install' );
		...
		$myMess = '<span class="label label-danger"">'. $mainMessage . '</span><br>';
 	...
 	private function _searchPreferencesPageForNewlyInstalledApps() {
 		$appToSetup='';
self::logMessage( $this->installing, 'install' );
...
self::logMessage( 'checking constant '.$constant,'install' );
					if (defined($constant)){
						$myConstant = constant($constant);
self::logMessage( 'constant '.$constant.' exists','install' );
												if ( $myConstant<1 ) {
							...
							$_SESSION['joobi']['config_urls'][] = $url;
														if (empty($appToSetup)){
self::logMessage( 'redirecting to '.$url,'install' );
