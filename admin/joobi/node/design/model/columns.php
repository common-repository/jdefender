...
class Design_Columns_model extends WModel {
	...
	function addValidate() {
		...
		$this->name = WGlobals::filter( $this->name, 'alphanumeric' );
	...
	function extra() {
		...
		if ( !empty($this->_updatedColumn) ) {
			...
			if ( empty($column['name']) ) $column['name'] = $this->_updatedColumn->name;
	...
	private function _addForeign() {
		...
		if ( !empty($ref_dbtid) ) {
			...
			$libraryForeignM = WModel::get( 'library.foreign' );
			...
			$libraryForeignM->setVal('ondelete',$ondelete);
			$libraryForeignM->setVal('onupdate',$onupdate);
			$libraryForeignM->setVal('ref_dbtid',$ref_dbtid);
			$libraryForeignM->setVal('ref_feid',$ref_feid);
			$libraryForeignM->setVal('dbtid',$this->dbtid);
			$libraryForeignM->setVal('feid',$this->dbcid);
			$libraryForeignM->setVal( 'namekey', 'FK_' . $tableName.'_'.$this->name);
			$libraryForeignM->setVal('map',$this->name);
			$libraryForeignM->setVal('map2',$this->name);
			$libraryForeignM->insert();
			$message = WMessage::get();
