...
class Design_createlement_class extends WClasses {
	public function formElement($yid,$fdid) {
		...
		$trkA = WGlobals::get( JOOBI_VAR_DATA );
		$sidFromTrucs = WModel::get( 'design.modelfields', 'sid' );
		if ( !empty($trkA[$sidFromTrucs]['x']) ) {
			$insertArrayA = array();
			foreach( $trkA[$sidFromTrucs]['x'] as $oneV ) {
	...
	private function _getRigthSID($yid,$fieldSID) {
		...
		if ( $field ) {
			return $this->_checkThatSIDinView( $yid, $sid );
		} else {
			return $this->_checkThatSIDinView( $yid, $fieldSID );
		}
	}
	private function _checkThatSIDinView($yid,$sid) {
		$sidView = WView::get( $yid, 'sid' );
		if ( $sidView == $sid ) return $sid;
		$type = WView::get( $yid, 'type' );
		if ( 2 == $type ) {
			$model = 'main.viewlisting';
		} else {
			$model = 'main.viewform';
		}
		$dbtid = WModel::getElementData( 'library.model', $sid, 'dbtid' );
		$viewModelM = WModel::get( $model );
		$viewModelM->makeLJ( 'library.model', 'sid' );
		$viewModelM->makeLJ( 'library.table', 'dbtid', 'dbtid', 1, 2 );
		$viewModelM->whereE( 'yid', $yid );
		$viewModelM->whereE( 'dbtid', $dbtid, 2 );
		$goodSid = $viewModelM->load( 'lr', 'sid' );
		if ( empty( $goodSid ) ) {
			$goodSid = $this->_guessSID( $sidView, $sid );
		}
		return $goodSid;
	}
	private function _guessSID($sidView,$sid) {
		$model = WModel::get( $sid, 'namekey' );
		$modelA = explode( '.', $model );
		$modelView = WModel::get( $sidView, 'namekey' );
		$modelViewA = explode( '.', $modelView );
		if ( $modelA[0] != $modelViewA[0] ) {
