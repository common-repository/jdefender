...
class Design_System_class extends WClasses {
	...
	function _deleteFile() {
		...
		else $this->fileP = JOOBI_DS_USER . 'node/' . $tabExt[0] . '/' . $this->folder . '/' . $tabExt[1].'.php';
	...
	function filePathSetVar($subfolder='') {
		...
		if ( !empty($subfolder) ) $this->folder = $this->folder. '/' . $subfolder;
				$tabExt = explode('.',$this->fileLocation);
		if ( !empty($tabExt[0]) && empty($tabExt[1]) ) $tabExt[1]='node';
		...
		$this->fileP = JOOBI_DS_USER . 'node/' . $tabExt[0] . '/' .  $this->folder . '/' . $tabExt[1] . '.' . $this->contentType;
