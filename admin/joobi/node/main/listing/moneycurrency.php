...
class WListing_Coremoneycurrency extends WListings_default{
	function create() {
		...
		if ( !defined('CURRENCY_USED') ) {
			...
			$dropdownPL->params = $this->element;
						$dropdownPL->name = JOOBI_VAR_DATA . '[' . $this->modelID . '][curid]';
	...
	public function advanceSearch() {
		...
		if ( false == $this->content ) return false;
	...
	private function _renderPriceFormHTML($lid) {
		if ( !isset(self::$complexMapA[$lid])  ) return false;
	...
	public function searchQuery(&$model,$element,$searchedTerms=null,$operator=null) {
		$lid = $element->lid;
		$signExtra = substr( $lid, -2 );
		$sign = ( '-s' == $signExtra ? '>=' : '<=' );
				$this->createComplexIds( $lid, $element->map . '_' . $element->sid . $signExtra );
		...
		if ( $addFitler && !empty($defaultValue) && is_numeric($defaultValue) ) {
			if ($range) $model->where( $element->map, $sign, $defaultValue, $element->asi );
			else $model->whereSearch( $element->map, $defaultValue, $element->asi, $sign, $operator );
		}
		return true;
	}
}