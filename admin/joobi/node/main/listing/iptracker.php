...
class WListing_Coreiptracker extends WListings_default{
	...
	function create() {
		if ( !empty($this->value) && '0.0.0.0' != $this->value ) {
																		$iptrackerLookupC = WClass::get( 'security.lookup', null, 'class', false );
					if ( !empty($iptrackerLookupC) ) {
						$cty = $iptrackerLookupC->ipInfo( $this->value, 'ctyid' );
					} else {
						$cty = $this->getValue( 'ctyid' );
					}
								if ( !empty($cty) ) $country = WModel::getElementData( 'countries', $cty, 'name' );
				if ( !empty($country) ) {
					...
					static $ipClass=null;
					if ( !isset($ipClass) ) $ipClass = WClass::get( 'security.lookup', null, 'class', false);
