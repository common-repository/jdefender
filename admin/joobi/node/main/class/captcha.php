...
class Main_Captcha_class extends WClasses {
	...
	public function needInputBox() {
		$captchatype = WPref::load( 'PUSERS_NODE_USECAPTCHATYPE' );
	...
	public function checkProcedure($useCaptcha) {
		...
		if ( $captchaID && is_numeric($captchaID) ) {
			if ( ! $this->verify($captchaID) ) {
				...
				return $message->historyE('1206732406SEHE');
				return false;
			}		} else {
			$message = WMessage::get();
			$this->_saveEditorStuff();
	...
	private function _saveEditorStuff() {
		...
		if ( !empty($requestA) ) {
			$len = strlen( JOOBI_VAR_DATA ) + 6;
			foreach( $requestA as $oneK => $oneV ) {
				if ( 'zdtr_' . JOOBI_VAR_DATA . '_' == substr( $oneK, 0, $len ) ) {
