...
class Main_Confighelper_class extends WClasses {
	...
	public function renderConfig() {
		...
		if ( !empty($layout) ) $html .= $layout->make();
		return $html;
	}
	public function renderViewDetails() {
		...
		$btnO->text = WText::t('1357697641QROB');
		$btnO->id = 'viewedt' . $yid;
		$btnO->type = 'infoLink';
		$btnO->extraTags = ' target="_blank"';
		$btnO->link = WPage::linkAdmin( 'controller=main-view&task=edit&eid=' . $yid );
		$btnO->color = 'primary';
		$myView .= ' ' . WPage::renderBluePrint( 'button', $btnO );
		$btnO = WPage::newBluePrint( 'button' );
		$btnO->text = WText::t('1214216703DDGQ');
		$btnO->extraTags = ' target="_blank"';
		$btnO->id = 'viewedt' . $yid;
		$btnO->type = 'infoLink';
		$lk = ( 2 == WView::get( $yid, 'type' ) ? 'listing' : 'form' );
		$btnO->link = WPage::linkAdmin( 'controller=main-' . $lk . '&yid=' . $yid );
		$btnO->color = 'success';
		$myView .= WPage::renderBluePrint( 'button', $btnO );
		...
		if ( !empty(WView::$removedElementsA) ) {
			$elementS = '';
			foreach( WView::$removedElementsA as $viewElem ) {
				if ( !empty($elementS) ) $elementS .= '<br>';
				$elementS .= implode( '<br>', $viewElem );
			}			if ( !empty($elementS) ) $value2DisplayA[WText::t('1468618442RSQV')] = $elementS;
		}		if ( !empty(WView::$changeElementsA) ) {
			$chElemA = array();
			foreach( WView::$changeElementsA as $v ) {
				$chElemA[$v->namekey] = true;
			}			$value2DisplayA[WText::t('1468618442RSQW')] = implode( '<br>', array_keys($chElemA) );
		}		if ( !empty(WView::$removedConditionsA) ) {
			$value2DisplayA[WText::t('1468618442RSQX')] = implode( '<br>', WView::$removedConditionsA );
		}		if ( !empty(WView::$removedPicklistValuesA) ) {
			$value2DisplayA[WText::t('1468618442RSQY')] = implode( '<br>', WView::$removedPicklistValuesA );
		}		
...
<div class="col-sm-10">' . $output . '</div>
	...
	private function _debuConvertURL() {
		...
		if ( !empty($finalURL) ) {
			$finalURL = WPage::linkNoSEF( $finalURL, 'standard' );
		}
		return $finalURL;
	}
	private function _isCore($yid) {
		...
		if ( empty($allNoneCoreA) ) {
			...
			$html .= '<br />UnCore elements:<br />' . implode( '<br />', $allNoneCoreA );
		}
		return $html;
	}
