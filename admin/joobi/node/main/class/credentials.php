...
public function picklistFromCategory(&$object,$category=0,$firstValuesA=array(),$defaultValue=true) {
	...
	if ( $defaultValue ) $object->addElement( 0, '- ' . WText::t('1458930386NLSL') . ' -' );
...
private function _loadCredentials($credentialsID=null,$type=null,$return=null) {
	...
	if ( !isset($credentialsA[$credentialsID]) ) {
		...
		if ( !empty($credentialsID) ) {
			if ( is_numeric($credentialsID) ) {
				$mainCredentialsM->whereE( 'crdid' , $credentialsID );
			} else {
				$mainCredentialsM->whereE( 'namekey' , $credentialsID );
			}		} elseif ( !empty($type) ) {
			if ( is_numeric( $type ) ) {
