...
class WForm_Coremoneycurrency extends WForm_text {
	...
	function create() {
		...
		if ( !empty($defaultCurrency) ) $this->value = WTools::format( $this->value, 'priceDecimal', $defaultCurrency, null, false, '', false );
		...
		if ( $multipleCurrency ) {
			...
			$formObject->hidden( JOOBI_VAR_DATA . '['.$this->modelID.'][curid]', $defaultCurrency );
