...
class Main_Page_tag {
	...
	public function process($object) {
		...
		foreach($object as $tag => $myTagO){
			...
			if ( empty($myTagO->url) ) {
				...
				$message->userW('1455637682QEKB');
				continue;
			}
			if (strpos($myTagO->url,'http://') === false){
				$myTagO->url = 'http://'.$myTagO->url;
			}
			$page = file_get_contents($myTagO->url);
			if ($page === false){
				$URL = $myTagO->url;
			...
			$this->_charset($page);
			$this->_linkWithoutSEF( $page, $myTagO->url );
			$css = $this->_extractCSS($page,$myTagO->url);
			$this->_replaceCSS($page,$css);
			$this->_removeScript($page);
			$myTagO->wdgtContent = $page;
	...
	private function _linkWithoutSEF(&$page,$urlPage) {
