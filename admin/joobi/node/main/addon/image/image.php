...
class Main_Image_addon extends WClasses {
	...
	private $dic = '';
	private $abcfixe = 1;
	private $abc = 'abcdefhjkmnpqrstwxyz2345678';		private $defaultlg = 'en';
	...
	function generate(&$html) {
		...
		$params = array();
				if ( $this->options['nbcached'] ) {
						$handler = WGet::folder();
			$captchafolder = JOOBI_DS_MEDIA . 'images/captcha';
			if ($handler->exist($captchafolder) ) {
								...
								if ($this->options['nbcached'] < $currentnb)
				{
					...
					while(!$ok)
					{
						...
						if (!is_object($infos)) {
							$file = WGet::file();
							$file->delete(JOOBI_DS_MEDIA . 'images/captcha/' . $image);
							$i++;
							continue;
						}
					...
					$sizeinfos = getimagesize($captchafolder. '/' . $infos->image);
	...
	public function clean($infos) {
		...
		return $file->delete( JOOBI_DS_MEDIA . 'images/captcha/' . $infos->image );
	}
		...
		if ($this->randomtext) {
			...
			$abc = '';
				...
				$lg = $sql->getName($user->lgid );
								$abcfolder = JOOBI_DS_MEDIA . 'alphabets/';
	...
	private function _initImage()
	{
		...
		{
			$folder = WGet::folder();
			$path = JOOBI_DS_MEDIA . 'images/background/';
	...
	private function _writeWord($key){
		...
		$fontfolder =  JOOBI_DS_MEDIA . 'fonts/';
				switch($direction)
		{
			case 'ltr':
			...
			$fontFile = $fontfolder . $use_font. '/' . $use_font.'.ttf';
	...
	private function _saveImage($format='png')
	{
		...
		$captch_folder = JOOBI_DS_MEDIA . 'images/captcha/';
		$folder = WGet::folder();
		...
		switch($format)
		{
			...
			case 'png':
				if (!function_exists('imagepng'))
					return false;
				$result = @imagepng( $this->image, $captch_folder . $name . '.png', 9 );
				break;
		}
		if ($result===false)
		{
	...
	private function _generateFootprint($nb,$siarray,&$size,$aarray,&$angle,$farray,&$font,$key)
	{
				...
				while($i<$nb)
		{
			...
			$this->imagettfbbox_t(($siarray?$size[$i]:$size),($aarray?$angle[$i]:$angle),$fontfolder . $cur_font. '/' . $cur_font.'.ttf',$key[$i]);
			$i++;
		}
	}
