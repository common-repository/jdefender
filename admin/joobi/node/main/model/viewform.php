...
class Main_Viewform_model extends WModel {
	...
	function addValidate() {
		...
		if ( 'output.customized' == $this->type && empty($this->p['filef']) ) {
						$name = $this->getChild( 'main.viewformtrans', 'name' );
			$nameky = str_replace( '-', '_', WGlobals::filter( $name, 'path' ) );
			$this->namekey . $nameky;
			$this->p['filef'] = $nameky;				
		}		
		return true;
	...
	function editExtra() {
		if ( !empty($this->_p['codeform']) ) {
			$wid = WView::get( $this->yid, 'wid' );
			$folder = WExtension::get( $wid, 'folder' );
						$location = JOOBI_DS_USER . 'custom/' . $folder .'/form/' . $this->_p['filef'] . '.php';
			$code = '<?php defined(\'JOOBI_SECURE\') or die(\'J....\');';
			$code .= "\n";
			$code .= 'class ' . ucfirst( $folder ). '_' . ucfirst( $this->_p['filef'] ) . '_form extends WForms_default {';
			$code .= "\n";
			$code .= $this->_p['codeform'];
			$code .= "\n";
			$code .= '}';
			$fileS = WGet::file();
			$fileS->write( $location, $code, 'overwrite' );
		}		
		return true;
	}	
	function validate() {
	...
	private function _defineNamekey() {
		...
		if ( empty($this->map) ) {
			...
			$mapNmae = str_replace( array( ']', 'c[', 'v[', 'x[', 'm[', 'p[', 'j[', 'f[' ), '', $this->map );
